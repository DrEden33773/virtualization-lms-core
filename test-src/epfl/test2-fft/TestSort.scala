...
trait MergeSort { this: Relat =>
  ...
  def splitOddEven[T](xs: List[T]): (List[T], List[T]) =
    (xs: @unchecked) match {
      case o :: e :: xt =>
  ...
  def mergeOddEven[T](odd: List[T], even: List[T]): List[T] =
    ((odd, even): @unchecked) match {
      case (Nil, Nil) =>
        Nil
      case ((o :: os), (e :: es)) =>
  ...
  def merge(xs: List[Rep[Double]]): List[Rep[Double]] = (xs: @unchecked) match {
    ...
    case _ =>
      ...
      val (odd2, even2) = odd1 zip even1 map { case (x, y) =>
        (min(x, y), max(x, y))
      } unzip;

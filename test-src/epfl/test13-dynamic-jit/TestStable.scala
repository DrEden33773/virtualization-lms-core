...
trait CellOpsExp extends CellOps with BaseExp with StaticDataExp {
  ...
  def cell[T: Typ](tag: String): Cell[T] = staticData(
    new RCell[T](tag)
  ) // reflectMutable(CellInit(tag, x))
  def infix_set[T: Typ](c: Cell[T], x: Rep[T]): Rep[Unit] =
    reflectWrite(c)(CellSet(c, x))
...
trait ScalaGenCellOps extends ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case CellInit(tag, x) =>
      emitValDef(
        sym,
        "scala.lms.epfl.test13.RCell[" + remap(x.tp) + "](\"" + tag + "\")"
      )
...
trait CompileDyn extends Base with Compile {
  ...
  def dcompile[A: Typ, B: Typ](fv: List[Rep[Any]])(
      f: Rep[A] => Rep[B]
  ): Rep[A => B]

  def dcompile[A: Typ, B: Typ](f: Rep[A] => Rep[B]): Rep[A => B] =
    dcompile(freesyms(f))(f)

  def dlet[A: Typ, B: Typ](x: Rep[A], fv: List[Rep[Any]])(
      f: A => Rep[B]
  ): Rep[B]

  def dlet[A: Typ, B: Typ](x: Rep[A])(f: A => Rep[B]): Rep[B] =
    dlet(x, freesyms(f))(f)
...
trait CompileDynExp
  ...
  def dcompile[A: Typ, B: Typ](
      fv: List[Exp[Any]]
  )(f: Rep[A] => Rep[B]): Rep[A => B] = {
  ...
  def dlet[A: Typ, B: Typ](x: Exp[A], fv: List[Exp[Any]])(
      f: A => Rep[B]
  ): Rep[B] = {

    // compile { u: Rep[Unit] => f(x) }  <--- x is runtime value

    val fc = dcompileInternal[Unit, A, B](
      x :: fv,
      (u, v) => v.head.asInstanceOf[A]
    )(f) // don't really want x as free var but need lower bound on sym id for fresh ones
    unchecked[B](fc, ".apply(())")
  }

  def dcompileInternal[U: Typ, A, B: Typ](
      fv: List[Exp[Any]],
      g: (Rep[U], List[Any]) => A
  )(f: A => Rep[B]): Rep[U => B] = {
    ...
    val callback = { (fvVals: List[Any]) =>
      ...
      this.nVars = maxid
      compile { x: Rep[U] =>
        (fv zip fvVals).foreach { case (si: Sym[_], xi) =>
          createDefinition(si, StaticData(xi))
        }
        f(g(x, fvVals))
      }
    }
...
trait StableVars
  ...
  case class ReadValue[A: Typ, B](
      s: RCell[A],
      f: A => Continue[B],
      fv: List[Rep[Any]]
  ) extends Continue[B] { val m = typ[A] }

  def readValue[A: Typ, B](s: RCell[A])(f: A => Rep[B]) =
    ReadValue(s, (x: A) => Done(f(x)), freesyms(f))
  def readOneValue[A: Typ, B](s: RCell[A])(f: A => Continue[B]) =
    ReadValue(s, f, freesyms(f))
  ...
  def testStable2 = withOutFileChecked(prefix + "stable2") {
    trait Prog extends DSL {
      def test() = {
        ...
        println(
          f(9)
        ) // find out s has changed, triggers OSR compilation, invalidates compiled method

...
import test7.{
  ...
  ArrayLoopsFatExp,
  ScalaGenArrayLoops,
  ScalaGenFatArrayLoopsFusionOpt
}
...
trait ComplexArith extends PrimitiveOps with ComplexBase with OverloadHack {
  ...
  def infix_+(x: Rep[Complex], y: Rep[Complex])(implicit
      o: Overloaded1
  ): Rep[Complex] = Complex(x.re + y.re, x.im + y.im)
  def infix_-(x: Rep[Complex], y: Rep[Complex])(implicit
      o: Overloaded1
  ): Rep[Complex] = Complex(x.re - y.re, x.im - y.im)
...
trait ComplexBase extends StructOps {
  ...
  def Complex(r: Rep[Double], i: Rep[Double]): Rep[Complex] = new Record {
    val re = r; val im = i
  }
}
...
trait StructExpOptLoops
    extends StructExpOptCommon
    with ArrayLoopsExp
    with PrimitiveOpsExp {

  case class ArraySoaTag[T](base: StructTag[T], len: Exp[Int])
      extends StructTag[T]

  override def simpleLoop[A: Typ](size: Exp[Int], v: Sym[Int], body: Def[A])(
      implicit pos: SourceContext
  ): Exp[A] = body match {
    case ArrayElem(Block(Def(Struct(tag: StructTag[A], elems)))) =>
      struct[A](
        ArraySoaTag[A](tag, size),
        elems.map(p =>
          (
            p._1,
            simpleLoop(size, v, ArrayElem(Block(p._2)))(p._2.tp.arrayTyp, pos)
          )
        )
      )
    case ArrayElem(Block(Def(ArrayIndex(b, `v`))))
        if infix_length(b)(
          mtype(manifestTyp[Any] /*FIXME:wrong type!*/ )
        ) == size =>
      b.asInstanceOf[Exp[A]]
  ...
  override def infix_at[T: Typ](a: Rep[Array[T]], i: Rep[Int]): Rep[T] =
    a match {
      case Def(
            Struct(
              ArraySoaTag(tag, len),
              elems: Iterable[(String, Rep[Array[T]])]
            )
          ) =>
        def unwrap[A](m: Typ[Array[A]]): Typ[A] = m.typeArguments match {
          ...
          case _ =>
            if (m.isArray)
              mtype(simpleClassTyp(m.runtimeClass.getComponentType))
            else {
              printerr("warning: expect type Array[A] but got " + m);
              mtype(manifestTyp[Any])
            }
        }
        struct[T](
          tag.asInstanceOf[StructTag[T]],
          elems.map(p => (p._1, infix_at(p._2, i)(unwrap(p._2.tp))))
        )
...
class TestStruct extends FileDiffSuite {
  ...
  trait Impl
    ...
    val codegen = new ScalaGenArrayLoops
      with ScalaGenStruct
  ...
  trait ImplFused
      ...
      with PrintExp { self =>
    override val verbosity = 1
    val codegen = new ScalaGenFatArrayLoopsFusionOpt
      ...
      override def shouldApplyFusion(currentScope: List[Stm])(
          result: List[Exp[Any]]
      ): Boolean = true
    }
    {
    ...
    withOutFile(prefix + "struct2") {
      ...
      trait Prog extends DSL {
        def test(x: Rep[Int]) = {
          ...
          val vector1 = array(100) { i =>
            Complex(i.toDouble, 0.0 - i.toDouble)
          }
          val vector2 = array(100) { i =>
            Complex(0.0 - i.toDouble, i.toDouble)
          }
    ...
    withOutFile(prefix + "struct3") {
      ...
      trait Prog extends DSL {
        def test(x: Rep[Int]) = {
          ...
          def infix_map[A: Typ, B: Typ](c: Rep[Array[A]], f: Rep[A] => Rep[B]) =
            array(c.length) { i => f(c.at(i)) }
      ...
      new Prog with ImplFused {
        ...
        override def double_minus(x: Exp[Double], y: Exp[Double])(implicit
            pos: SourceContext
        ) = (x, y) match {
          case (x, Def(DoubleMinus(Const(0.0), y))) => infix_+(x, y)
          case _                                    => super.double_minus(x, y)
        }
        override def double_plus(x: Exp[Double], y: Exp[Double])(implicit
            pos: SourceContext
        ) = (x, y) match {
          case (Const(0.0), y) => y
    ...
    withOutFile(prefix + "struct4") {
      trait Prog extends DSL {
        ...
        def test(x: Rep[Int]) = {
          ...
          var vvar =
            vector1 // force access outside conditional, otherwise construction will be moved inside, defeating purpose of test

...
class TestLambdalift extends FileDiffSuite {
  ...
  def testLambdalift1 = {
    ...
    withOutFile(prefix + "lambdalift1") {
      ...
      new NestLambdaProg1 with CoreOpsPkgExp with FunctionsExp with PrintExp {
        self =>
          ...
          override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
            case e @ Lambda(fun, x, y) =>
              focusBlock(y) {
                ...
                stream.println(
                  "val " + quote(sym) + " = new Anonfun_" + quote(
                    sym
                  ) + "(" + free.map(quote).mkString(",") + ")"
                )
              }
        ...
        val codegen2 = new ScalaGenBla
          ...
          def emitFocused[A, B](
            ...
            withStream(out) {
              stream.println(
                "class " + name + "(" + params.map(quote).mkString(",") + ") {"
              )
        ...
        val codegenInner: ScalaGenBla {
          val IR: self.type; type Block[+T] = self.Block[T]
        } = codegen2

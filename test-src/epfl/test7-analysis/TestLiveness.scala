    ...
    focusExactScope(result) { levelScope =>
      ...
      def usesOf(s: Sym[Any]): List[TP[Any]] = levelScope.flatMap {
        case TP(
              s1,
              Reify(rhs1, _, _)
            ) => // reify nodes are eliminated, so we need to find all uses of the reified thing
      ...
      for (TP(sym, rhs) <- levelScope) {
        ...
        rhs match {
          ...
          case _                    =>
            ...
            if (killed.nonEmpty)
              stream.println("// kill: " + killed.map(quote).mkString(", "))
    ...
    withOutFile(prefix + "liveness1") {
      new LiveProg with CoreOpsPkgExp with ArraysExp with PrintExp { self =>
        ...
        val codegen = new ScalaGenPrimitiveOps
          with ScalaGenArrays
          with ScalaGenPrint
          with Liveness { val IR: self.type = self }

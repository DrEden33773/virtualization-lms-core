...
trait ArrayLoops extends Loops with OverloadHack { this: PrimitiveOps =>
  ...
  def sum(
      ...
      f: Rep[Int] => Rep[Double]
  ): Rep[Double] // TODO: make reduce operation configurable!
  def arrayIf[T: Typ](shape: Rep[Int])(
      f: Rep[Int] => (Rep[Boolean], Rep[T])
  ): Rep[Array[T]]
  def sumIf(shape: Rep[Int])(
      f: Rep[Int] => (Rep[Boolean], Rep[Double])
  ): Rep[Double] // TODO: make reduce operation configurable!
  def flatten[T: Typ](shape: Rep[Int])(
      f: Rep[Int] => Rep[Array[T]]
  ): Rep[Array[T]]
...
trait ArrayLoopsExp extends LoopsExp { this: PrimitiveOpsExp =>
  ...
  def arrayIf[T: Typ](
      shape: Rep[Int]
  )(f: Rep[Int] => (Rep[Boolean], Rep[T])): Rep[Array[T]] = {
    val x = fresh[Int]
    // val (c,y) = f(x)
    ...
    simpleLoop(
      ...
      x,
      ArrayIfElem(c, y)
    ) // TODO: simplify for const true/false
  }

  def sumIf(
      shape: Rep[Int]
  )(f: Rep[Int] => (Rep[Boolean], Rep[Double])): Rep[Double] = {
    val x = fresh[Int]
    // val (c,y) = f(x)
    var c: Rep[Boolean] = null
    val y = reifyEffects { val p = f(x); c = p._1; p._2 }
  ...
  def flatten[T: Typ](
      shape: Rep[Int]
  )(f: Rep[Int] => Rep[Array[T]]): Rep[Array[T]] = {
    val x = fresh[Int]
    val y = reifyEffects(f(x))
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case SimpleLoop(s, i, ArrayElem(y)) if f.hasContext =>
      implicit def anyTyp: Typ[Any] = manifestTyp // FIXME: wrong type
      array(f(s)) { j =>
        f.asInstanceOf[
          AbstractSubstTransformer { val IR: ArrayLoopsExp.this.type }
        ].withSubstScope(i -> j) {
          f.reflectBlock(y)
        }
      }
  ...
  override def mirrorFatDef[A: Typ](e: Def[A], f: Transformer)(implicit
      pos: SourceContext
  ): Def[A] = (e match {
...
trait ScalaGenArrayLoops extends ScalaGenLoops {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case SimpleLoop(s, x, ArrayElem(y)) =>
      stream.println(
        "val " + quote(sym) + " = LoopArray(" + quote(s) + ") { " + quote(
          x
        ) + " => "
      )
      emitBlock(y)
    ...
    case SimpleLoop(s, x, ReduceElem(y)) =>
      stream.println(
        "val " + quote(sym) + " = LoopReduce(" + quote(s) + ") { " + quote(
          x
        ) + " => "
      )
      emitBlock(y)
    ...
    case SimpleLoop(s, x, FlattenElem(y)) =>
      stream.println(
        "val " + quote(sym) + " = LoopFlatten(" + quote(s) + ") { " + quote(
          x
        ) + " => "
      )
      emitBlock(y)
    ...
    case SimpleFatLoop(s, x, rhs) =>
      for ((l, r) <- sym zip rhs) {
        r match {
          case ArrayElem(y) =>
            stream.println(
              "var " + quote(l) + " = new Array[" + remap(
                getBlockResult(y).tp
              ) + "](" + quote(s) + ")"
            )
          case ReduceElem(y) =>
            stream.println(
              "var " + quote(l) + ": " + remap(getBlockResult(y).tp) + " = 0"
            )
          case ArrayIfElem(c, y) =>
            stream.println(
              "var " + quote(l) + " = new ArrayBuilder[" + remap(
                getBlockResult(y).tp
              ) + "]"
            )
...
//      }
      ...
      for ((l, r) <- sym zip rhs) {
        r match {
          case ArrayElem(y) =>
            stream.println(
              quote(l) + "(" + quote(ii) + ") = " + quote(getBlockResult(y))
            )
          case ReduceElem(y) =>
            stream.println(quote(l) + " += " + quote(getBlockResult(y)))
          case ArrayIfElem(c, y) =>
            stream.println(
              "if (" + quote( /*getBlockResult*/ (c)) + ") " + quote(
                l
              ) + " += " + quote(getBlockResult(y))
            )
          case ReduceIfElem(c, y) =>
            stream.println(
              "if (" + quote( /*getBlockResult*/ (c)) + ") " + quote(
...
trait Arrays extends Base with PrimitiveOps with OverloadHack {
  ...
  def infix_+(a: Rep[Array[Int]], b: Rep[Array[Int]])(implicit
      o: Overloaded1
  ): Rep[Array[Int]]
}
...
trait ArraysExp extends Arrays with PrimitiveOpsExp with EffectExp {
  ...
  case class ArrayWrite(a: Rep[Array[Int]], x: Rep[Int], v: Rep[Int])
      extends Def[Array[Int]]
  case class ArrayPlus(a: Rep[Array[Int]], b: Rep[Array[Int]])
      extends Def[Array[Int]]
  def zeroes(n: Rep[Int]) = ArrayZero(n)
  def infix_update(a: Rep[Array[Int]], x: Rep[Int], v: Rep[Int]) =
    ArrayWrite(a, x, v)
  def infix_+(a: Rep[Array[Int]], b: Rep[Array[Int]])(implicit o: Overloaded1) =
    ArrayPlus(a, b)
}
...
trait ScalaGenArrays extends ScalaGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case ArrayPlus(a, b) =>
      ...
      stream.println(
        "arrayPlus(" + quote(sym) + "," + quote(a) + "," + quote(b) + ")"
      )
    case _ => super.emitNode(sym, rhs)
  }
}

...
trait ScalaGenFatArrayLoopsFusionOpt
    extends ScalaGenArrayLoopsFat
  ...
  override def applyAddCondition(e: Def[Any], c: List[Exp[Boolean]]) =
    e match { // TODO: should c be list or not?
...
trait FusionProg
  ...
  override implicit def repIntToRepDouble(x: Rep[Int]): Rep[Double] =
    x.asInstanceOf[Rep[Double]]
  ...
  def test(x: Rep[Unit]) = {
    ...
    def variance(x: Rep[Array[Double]]) = sum(x.length) { i =>
      square(x.at(i))
    } / x.length - square(mean(x))
...
class TestFusion extends FileDiffSuite {
  ...
  def testFusion1 = {
    withOutFile(prefix + "fusion1") {
      new FusionProg with CoreOpsPkgExp with ArrayLoopsExp with PrintExp {
        self =>

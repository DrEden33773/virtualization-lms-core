...
trait TestDSL extends BaseExp with LiftAll {
  ...
  case class IfThenElse[T](c: Rep[Boolean], a: BlockStm[T], b: BlockStm[T])
      extends Def[T]

  type BwdInfo = Effects

  case class WaitBwd[T](
      e: Effects,
      body: (Sym[T], BwdInfo) => (List[Stm], BwdInfo)
  ) extends Def[T]
    ...
    if (c == Const(true)) a
      ...
      onValuesEffectsNeeded(getEffects(ab) ++ getEffects(bb)) { (s, bw) =>
        ...
        if (au == bu && ae == be)
          ...
          (
            List(createDefinition(s, IfThenElse(c, au, bu))),
            ae ++ be ++ List(c.toString)
          )
      }
  ...
  def onValuesEffectsNeeded[T](
      e: Effects
  )(body: (Sym[T], BwdInfo) => (List[Stm], Effects)): Exp[T] = {
  ...
  def reflectPure[T](d: => Def[T]) = {
    onValuesEffectsNeeded(List()) { (s, bw) =>
      if (bw contains s.toString)
        (
          List(createDefinition(s, d)),
          (bw filterNot (_ == s.toString)) ++ (syms(d) map (_.toString))
        )
      else
        (List(), bw)
    }
  ...
  def emitStm[T](stm: Stm): Unit = stm match {
    case TP(sym, IfThenElse(c, a, b)) =>
      println(sym + " = if (" + c + ")"); emitBlock(a); println("else");
      emitBlock(b)
  ...
  def flowBwd[T](e: Effects, block: BlockStm[T]) = {
    ...
    val stms2 = block.stms.reverse.flatMap {
      case TP(
            sym,
            WaitBwd(e, r: ((Sym[Any], BwdInfo) => (List[Stm], BwdInfo)))
          ) => // FIXME: type annot?

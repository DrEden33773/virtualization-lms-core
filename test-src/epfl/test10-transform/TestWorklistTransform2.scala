...
trait FWTransform2
  ...
  implicit def toAfter[A: Typ](x: Def[A]) = new {
    def atPhase(t: MyWorklistTransformer)(y: => Exp[A]) =
      transformAtPhase(x)(t)(y)
  }
  implicit def toAfter[A](x: Exp[A]) = new {
    def atPhase(t: MyWorklistTransformer)(y: => Exp[A]) =
      transformAtPhase(x)(t)(y)
  }
  ...
  def transformAtPhase[A](
      x: Exp[A]
  )(t: MyWorklistTransformer)(y: => Exp[A]): Exp[A] = {
    t.register(x)(y)
    x
  }
...
trait VectorExpTrans2
    extends FWTransform2
    with VectorExp
    with ArrayLoopsExp
    ...
    with BooleanOpsExp
    with EqualExpOpt
    with StructExp // with VariablesExpOpt
  ...
  def vfromarray[A: Typ](x: Exp[Array[A]]): Exp[Vector[A]] =
    struct(ClassTag[Vector[A]]("Vector"), "data" -> x, "length" -> x.length)
  def vtoarray[A: Typ](x: Exp[Vector[A]]): Exp[Array[A]] =
    field[Array[A]](x, "data")

  // FIXME: manifests are wrong -- need to take from Def
  override def onCreate[A: Typ](s: Sym[A], d: Def[A]) = (d match {
    case VectorZeros(n) =>
      s.atPhase(xform) { vzeros_xform(xform(n)).asInstanceOf[Exp[A]] }
    case VectorApply(a, x) =>
      s.atPhase(xform) {
        vapply_xform(xform(a), xform(x))(mtyp1[A]).asInstanceOf[Exp[A]]
      }
    case VectorLength(x) =>
      s.atPhase(xform) {
        vlength_xform(xform(x))(mtyp1[A]).asInstanceOf[Exp[A]]
      }
    case VectorPlus(a, b) =>
      s.atPhase(xform) { vplus_xform(xform(a), xform(b)).asInstanceOf[Exp[A]] }
...
class TestForward2 extends FileDiffSuite {
  ...
  trait Impl
      extends DSL
      with VectorExpTrans2
      with PrimitiveOpsExp
      with OrderingOpsExpOpt
      with BooleanOpsExp
      with EqualExpOpt
      with StructFatExpOptCommon // with VariablesExpOpt
      with IfThenElseExpOpt
      with WhileExpOptSpeculative
      with RangeOpsExp
      with PrintExp { self =>
    ...
    val codegen = new ScalaGenVector
      ...
      with ScalaGenPrint
      with ScalaGenFatStruct { val IR: self.type = self }
    ...
    trait Prog extends DSL with Impl {
      def test(x: Rep[Int]) = {
        ...
        val a = vplus(
          z,
          y
        ) // this is disabled right now but could trigger rewriting before going to arrays
        val b = vplus(z, a)
        print(b)
      }
    }

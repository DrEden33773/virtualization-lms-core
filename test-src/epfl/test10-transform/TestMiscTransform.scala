...
trait SimpleBlockTransformer extends internal.FatBlockTraversal {
  val IR: Expressions
    with Effects
    with FatExpressions
    with Transforming // with LoopsFatExp with IfThenElseFatExp
  import IR._
  ...
  def transformBlockFocused[A](block: Block[A]): Block[A] = {
    focusExactScope(block) { levelScope =>
      ...
      levelScope foreach { stm =>
        ...
        reflectSubGraph(
          stms3
        ) // may fail if we're redefining a symbol that already exists
      }
      Block(block.res)
    }
  }

  def transformStm(stm: Stm): List[Stm] =
    stm match { // override this to implement custom traversal
      case TP(s, d) =>
        val trans = new AbstractTransformer {
          val IR: SimpleBlockTransformer.this.IR.type =
            SimpleBlockTransformer.this.IR
...
trait NestedBlockTransformer extends internal.FatBlockTraversal {
    ...
    focusExactScope(block) { levelScope =>
      ...
      levelScope foreach { stm =>
        ...
        val stms4 = stms3 map { s =>
          if (changedSubst)
            println(
              "*** potential problem: changed subst in the middle of iterating over transformed stms"
            )
  ...
  def transformStm(stm: Stm): List[Stm] =
    stm match { // override this to implement custom traversal
      case TP(s, d) =>
        val trans = new AbstractTransformer {
          val IR: NestedBlockTransformer.this.IR.type =
            NestedBlockTransformer.this.IR
...
trait MirrorBlockTransformer extends internal.FatBlockTraversal {
  ...
  def mirrorBlock[A](block: Block[A]): Exp[A] = {
    focusBlock(block) {
      focusExactScope(block) { levelScope =>
        ...
        val res = getBlockResult(
          Block(transformExp(block.res))
        ) // strip translated Reify...
        subst = saveSubst
        res
      }
    }
  ...
  def transformStm(stm: Stm): Exp[Any] =
    stm match { // override this to implement custom traversal
      case TP(s, d) =>
        val trans = new AbstractTransformer {
          val IR: MirrorBlockTransformer.this.IR.type =
            MirrorBlockTransformer.this.IR
          def apply[A](x: Exp[A]) = transformExp(x)
          override def apply[A](x: Block[A]) = transformBlock(x)
        }
  ...
  override def transformStm(stm: Stm): Exp[Any] =
    stm match { // override this to implement custom traversal
      case TP(s, d) =>
        ...
        val trans = new AbstractTransformer {
          val IR: MirrorRetainBlockTransformer.this.IR.type =
            MirrorRetainBlockTransformer.this.IR
          def apply[A](x: Exp[A]) = transformExp(x)
          override def apply[A](x: Block[A]) = transformBlock(x)
        }
        mirror(d, trans)(mtype(s.tp), mpos(s.pos))
...
class TestMisc extends FileDiffSuite {
  ...
  def testMisc2 = withOutFileChecked(prefix + "misc2") {
    ...
    p.globalDefsCache = Map.empty
    val trans =
      new SimpleBlockTransformer { // a + b --> b + a, but only in then-branches of an if-then-else
        val IR: p.type = p
  ...
  def testMisc3 = withOutFileChecked(prefix + "misc3") {
    ...
    p.globalDefsCache = Map.empty
    val trans =
      new NestedBlockTransformer { // a + b --> b + a, but only in then-branches of an if-then-else
        val IR: p.type = p
        import IR.{__newVar => _, _}
        override def initialDefs = graph
        var isInThenBranch = false
          ...
          case TP(s, Reflect(IfThenElse(c, a, b), u, es)) =>
            ...
            List(
              TP(
                s,
                Reflect(
                  IfThenElse(c2, a2, b2)(mtype(s.tp)),
                  u,
                  es map transformExp
                )
              )
  ...
  def testMisc4 = withOutFileChecked(prefix + "misc4") {
    ...
    p.globalDefsCache = Map.empty
    val trans =
      new MirrorBlockTransformer { // a + b --> b + a, but only in then-branches of an if-then-else
        val IR: p.type = p
        import IR.{__newVar => _, _}
        override def initialDefs = graph
        var isInThenBranch = false
  ...
  def testMisc5 = withOutFileChecked(prefix + "misc5") {
    ...
    val trans =
      new MirrorRetainBlockTransformer { // a + b --> b + a, but only in then-branches of an if-then-else
        val IR: p.type = p
        import IR.{__newVar => _, _}
        override def initialDefs = graph
        var isInThenBranch = false
    ...
    try {
      ...
      println(
        "// not resetting graph inbetween runs and smarter pruning of statements based on their inputs."
      )

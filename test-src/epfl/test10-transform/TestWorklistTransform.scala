...
trait FWTransform1
  ...
  protected override implicit def toAtom[A: Typ](d: Def[A])(implicit
      pos: SourceContext
  ): Exp[A] = { // override createDefinition instead?
    val in = syms(d)
    val actual = xform(in)
...
trait VectorExpTrans1
    extends FWTransform1
    with VectorExp
    with ArrayLoopsExp
  ...
  override def vzeros(n: Rep[Int]) = VectorZeros(n).atPhase(xform) {
    vfromarray(array(n) { i => 0.0 })
  }

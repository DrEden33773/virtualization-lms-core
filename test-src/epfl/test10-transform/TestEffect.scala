...
trait LibExp extends Lib with VectorExp with BaseFatExp with EffectExp {
  ...
  var subst: Map[Sym[_], Exp[_]] =
    Map() // need to hook this up with reifyEffects? does it ever need to be reset?
  ...
  override implicit def toAtom[A: Typ](
      d: Def[A]
    ...
    if (in != actual) {
      ...
      if (kill.nonEmpty) {
        ...
        val transitive = globalDefs collect {
          case e @ TP(s, rhs)
              if (s :: allAliases(rhs) intersect kill).nonEmpty =>
            e
        }
...
class TestEffects extends FileDiffSuite {
    ...
    trait Prog extends DSL {
      def test(x: Rep[Int]) = {
        ...
        vupdate(
          ...
          5,
          7.0
        ) // must undo cse: now x and y are different. also, as and bs are different now
        val u = vapply(as, 0)
        val v = vapply(bs, 0)
        print(u)
        print(v)
  ...
  def testEffects3 = withOutFileChecked(prefix + "effects3") { // test cse
    trait Prog extends DSL {
      def test(x: Rep[Int]) = {
        ...
        val e = vliteral(
          List(y)
        ) // assume that this operation is expensive (don't want to do it twice)
        print(e)
        vupdate(
          x,
          5,
  ...
  def testEffects4 =
    withOutFileChecked(prefix + "effects4") { // test mutable dce
      trait Prog extends DSL {
        def test(x: Rep[Int]) = {
          val x = vzeros(100)
          val y = x
          ...
          val bs = vliteral(
            List(y)
          ) // this one should be dce'd because it is never used
          vupdate(
            x,
            5,
            7.0
          ) // this will invalidate bs (anti-dep) but should not give rise to a hard dependency
          val u = vapply(as, 0)
          val v = vapply(bs, 0)
          print(u)
        }

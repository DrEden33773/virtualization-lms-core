...
class TestCGen extends FileDiffSuite {
  ...
  trait Impl
      ...
      with UncheckedOpsExp { self =>
    val codegen = new CCodeGenPkg
      with CGenVariables
      with CGenTupledFunctions
      with CGenUncheckedOps { val IR: self.type = self }
    def emitAll(): Unit = {
      ...
      rec.foreach { case (k, x) =>
        ...
        for ((_, v) <- rec)
          codegen.emitForwardDef(mtype(v.mA) :: Nil, v.name, stream)(
            mtype(v.mB)
          )

...
trait Extractors extends Base {
  ...
  def deconstruct[A, B: Typ](
      c: Class[A],
      f: A => Option[B],
      x: Rep[A]
  ): Option[Rep[B]]

  def deconstruct2[A, B: Typ, C: Typ](
      c: Class[A],
      f: A => Option[(B, C)],
      x: Rep[A]
  ): Option[(Rep[B], Rep[C])] = {
...
trait Matching extends Base {
  ...
  def evalMatch[A: Typ, B: Typ](
      x: Rep[A],
      alts: List[PartialFunction[Rep[A], Rep[B]]]
  ): Rep[B]
  ...
  implicit def switchable[A: Typ](x: Rep[A]) = new Switchable[A] {
    def switch[B: Typ](f: PartialFunction[Rep[A], Rep[B]]) =
      new Match[A, B](x, List(f)) // FIXME: won't find switch otherwise!!
  }

  class Match[A: Typ, B: Typ](
      x: Rep[A],
      alts: List[PartialFunction[Rep[A], Rep[B]]]
  ) {
    def orElse[D >: B](
        f: PartialFunction[Rep[A], Rep[D]]
    )(implicit mD: Typ[D]) = {
  ...
  def _match[A: Typ, B: Typ](
      x: Rep[A]
  )(cases: PartialFunction[Rep[A], Rep[B]]*): Rep[B] = {
...
trait MatchingExtractorsExpOpt extends MatchingExtractorsExp {
  override def construct[A: Typ, B: Typ](
      c: Class[A],
      f: B => A,
      x: Rep[B]
  ): Rep[A] = x match {
  ...
  override def deconstruct[A, B: Typ](
      c: Class[A],
      f: A => Option[B],
      x: Rep[A]
  ) = x match {
    case Def(Construct(`c`, y)) =>
      Some(y.asInstanceOf[Rep[B]]) // CHECK: `c` was _ : c.type

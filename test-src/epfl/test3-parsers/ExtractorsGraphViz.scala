...
//      stream.println("color=red")
  ...
  override def emitDeps(sym: Sym[Any], rhs: Def[Any], deps: List[Sym[Any]])(
      implicit stream: java.io.PrintWriter
  ) = rhs match {
    ...
    case Reify(x, effects) =>
      super.emitDeps(
        sym,
        rhs,
        deps filterNot (effects.contains(_))
      ) // TODO: use diff, but might have duplicates
      for (dep <- effects) {

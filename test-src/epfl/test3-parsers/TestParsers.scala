...
trait ParsersProgExp0 extends common.BaseExp with ParsersProg {
...
class TestParsers extends FileDiffSuite {
  ...
  def testParse1 = {
    withOutFile(prefix + "parse1") {
      object ParsersProgExp
          extends ParsersProgExp0
          with Matching
          with Extractors
          with MatchingExtractorsExpOpt
          with FunctionsExpUnfoldAll
          with FlatResult // with ControlOpt
          with DisableCSE {
        type Elem = Char
      ...
      val p = new ExtractorsGraphViz {
        val IR: ParsersProgExp.type = ParsersProgExp
      }

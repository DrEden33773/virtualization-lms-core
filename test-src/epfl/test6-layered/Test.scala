...
trait UtilExp extends BaseExp with Utils {
  ...
  def infix_+(a: Rep[String], b: Rep[Any])(implicit
      x: Overloaded1
  ): Rep[String] = StrCat(a, b)
  def infix_+(a: Rep[Any], b: Rep[String])(implicit
      x: Overloaded2
  ): Rep[String] = StrCat(a, b)
  def infix_+(a: String, b: Rep[Any])(implicit x: Overloaded4): Rep[String] =
    StrCat(Const(a), b)
  ...
  case class External[A: Typ](s: String, fmt_args: List[Exp[Any]] = List())
      extends Exp[A]
...
trait ScalaGenUtil extends ScalaGenBase {
  ...
  override def quote(x: Exp[Any]) = x match {
    case External(s: String, args: List[Exp[Any]]) =>
      s.format(args map (quote(_)): _*)
...
trait Vectors extends Utils {
  ...
  def infix_+(a: Rep[Vector], b: Rep[Vector])(implicit
      x: Overloaded3
  ): Rep[Vector]
}
...
trait VectorsExp extends Vectors with BaseExp { this: VectorsImpl =>
  ...
  def RandomVector(n: Exp[Int]) =
    doApply(vectorRandom, n) // random vectors are different...

  def infix_+(a: Exp[Vector], b: Exp[Vector])(implicit x: Overloaded3) =
    (a, b) match {
...
trait VectorsImplConst extends VectorsImpl {
  ...
  val vectorRandom =
    Const((n: Int) => new Array[Double](n)) // fill with random data...
  val vectorPlus =
    Const((p: (Vector, Vector)) => p._1) // actually do the addition
...
class TestVectors extends FileDiffSuite {
  ...
  def testVectors = {
    withOutFile(prefix + "vectors") {
      ...
      new VectorsProg
        with VectorsExp
        with VectorsImplExternal
        with CoreOpsPkgExp
        with CompileScala { self =>
        val codegen = new ScalaGenFunctions with ScalaGenUtil {

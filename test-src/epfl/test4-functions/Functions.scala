...
trait FunctionExpUnfoldRecursion
    extends FunctionsExpClever
    with FunctionsCanonical {
...
trait FunctionsExternalDef01 extends FunctionsExternalDef0 { // not used
  ...
  override def doLambda[A: Typ, B: Typ](
    ...
    createDefinition(
      funSym,
      DefineFun[A, B](Block(f(argSym)))(argSym)
    ) // FIXME: use reify (conflict with test3.effects)
    funSym
  }
...
trait FunctionsExternalDef2
    extends FunctionsCanonical
    with FunctionsExternalDef0 {

  override def lookupFun[A: Typ, B: Typ](
      f: Exp[A] => Exp[B]
  ): (Exp[A] => Exp[B]) = {
    var can = canonicalize(f)

...
trait MatcherProg { this: Matching with ListMatch =>
  ...
  def loop(p0: Input, s0: Rep[Input], pr: Input, sr: Rep[Input]): Rep[Boolean] =
    p0 match {
      case p :: pp =>
        s0 switch {
          case (s: Rep[Char]) :!: (ss: Rep[Input]) if s guard p => // matched p
            println("match")
            loop(pp, ss, pr, sr)
        } orElse { case s :!: ss => // no match for p
          println("no match")
          next(pr, sr)
        } orElse { case _ =>
          unit(false)
...
trait MatcherProgExp0 extends common.BaseExp with MatcherProg {
  this: Matching with ListMatch =>
...
class TestMatcher extends FileDiffSuite {
  ...
  def testMatcher1 = {
    withOutFile(prefix + "matcher1") {
      object MatcherProgExp
          extends MatcherProgExp0
          with Matching
          with Extractors
          with ListMatch
      ...
      println(r)
      val p = new ExtractorsGraphViz with FunctionsGraphViz {

...
trait AckProg {
  ...
  implicit def lam[A: Typ, B: Typ](f: Rep[A] => Rep[B]): Rep[A => B] = doLambda(
    f
  )
...
class TestAck extends FileDiffSuite {
  ...
  def testAck1 = {
    withOutFile(prefix + "ack1") {
      ...
      val p = new ScalaGenPrimitiveOps
        with ScalaGenEqual
        with ScalaGenIfThenElse
        with ScalaGenFunctionsExternal { val IR: AckProgExp.type = AckProgExp }

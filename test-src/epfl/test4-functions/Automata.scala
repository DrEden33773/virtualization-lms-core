...
case class Automaton[
    @specialized(Boolean, Char, Int) I,
    @specialized(Boolean, Char, Int) O
](out: O, next: I => Automaton[I, O])

case class NAutomaton[
    @specialized(Boolean, Char, Int) I,
    @specialized(Boolean, Char, Int) O
](out: O, next: I => List[NAutomaton[I, O]])
...
trait DFAOpsExp extends BaseExp with DFAOps { this: Functions =>
  ...
  case class DFAState(e: Rep[List[Any]], f: Rep[Char => DfaState])
      extends Def[DfaState]

  def dfa_flagged(e: Rep[Any])(rec: DIO): DIO = DFAFlagged(e, rec)
  def dfa_trans(e: Rep[List[Any]])(f: Rep[Char] => DIO): DIO =
    DFAState(e, doLambda(f))
...
trait ScalaGenDFAOps extends ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case DFAState(e, f) =>
      emitValDef(
        sym,
        "scala.lms.epfl.test4.Automaton(" + quote(e) + "," + quote(f) + ")"
      )
    case DFAFlagged(e, l) =>
      emitValDef(
        sym,
        quote(l) + ".copy(out = " + quote(e) + "::" + quote(l) + ".out)"
      )
...
trait NFAtoDFA extends DFAOps {
  this: PrimitiveOps with Functions with Equal with IfThenElse =>
...
def convertNFAtoDFA(in: NIO): DIO = {
  ...
  def guard(cond: CharSet, found: Boolean = false)(e: => NIO): NIO = {
    List(
      NTrans(
        cond,
        if (found) Some(unit("found").asInstanceOf[Rep[Unit]]) else None,
        () => e
      )
    )
  }
...
trait GAOps extends Base {
  ...
  type GIO =
    List[gTrans] // unstaged Automaton type = State type (State = list of possible transitions)
...
trait GAOpsExp extends BaseExp with GAOps {
  this: ListOps with IfThenElse with Functions =>
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case GTrans(e, f) =>
      emitValDef(
        sym,
        "List(" + remap(
          sym.tp.typeArguments(0)
        ) + /*scala.lms.epfl.test4.NAutomaton*/ "(" + quote(e) + "," + quote(
          f
        ) + "))"
      )
    case GCall(f, c) =>
      emitValDef(sym, quote(f) + ".flatMap(_.next.apply(" + quote(c) + "))")
    case GFlags(f) => emitValDef(sym, quote(f) + ".flatMap(_.out)")
    case _         => super.emitNode(sym, rhs)
  }
}

trait StepperOps extends DFAOps with Util {
  this: IfThenElse
    with ListOps
    with TupleOps
    with /*While with Variables with*/ Functions =>

  // Producers: produce values by executing state transitions

  def Prod[S: Typ](
      s0: Rep[S],
      cond: Rep[S] => Rep[Boolean],
      step: Rep[S] => Rep[S]
  ) =
  ...
  case class Prod2[S, O: Typ](st: Stepper2[Unit, S, O]) {
    ...
    def filter(p: Rep[O] => Rep[Boolean]) = {
      val stu = until(p).st
      def seek(sx: Rep[S]) =
        Prod2(Stepper2[Unit, S, S](sx, stu.cond, stu.yld, s => s)).run()
  ...
  case class Stepper2[I: Typ, S: Typ, O: Typ](
      s: Rep[S],
      cond: Rep[S] => Rep[Boolean],
      yld: Rep[I] => Rep[S] => Rep[S],
      res: Rep[S] => Rep[O]
  ) {
    def mfs = typ[S]
  ...
  def ftuple[S, S1](f1: S => S, f2: S1 => S1) = (ss: (S, S1)) =>
    (f1(ss._1), f2(ss._2))
  def frtuple[S: Typ, S1: Typ](f1: Rep[S] => Rep[S], f2: Rep[S1] => Rep[S1]) =
    (ss: Rep[(S, S1)]) => make_tuple2(f1(ss._1), f2(ss._2))
      ...
      def into[S2: Typ, O2: Typ](b: Stepper2[O1, S2, O2]) = {
        o.into(
          Stepper2[T, (S1, S2), O2](
            ...
            c =>
              ss =>
                if (p(c)) (a.s, b.yld(a.res(ss._1))(ss._2))
                else (a.yld(c)(ss._1), ss._2),
            ss => b.res(ss._2)
          )
        )

...
trait ArrayMutationExp
    extends ArrayMutation
    with PrimitiveOpsExp
    with ArrayLoopsExp {

  case class ArrayUpdate[T](a: Rep[Array[T]], i: Rep[Int], x: Rep[T])
      extends Def[Unit]
  case class ArrayMutable[T](a: Rep[Array[T]]) extends Def[Array[T]]
  case class ArrayClone[T](a: Rep[Array[T]]) extends Def[Array[T]]

  def infix_update[T: Typ](a: Rep[Array[T]], i: Rep[Int], x: Rep[T]) =
    reflectWrite(a)(ArrayUpdate(a, i, x))
  ...
  override def aliasSyms(e: Any): List[Sym[Any]] = e match {
    ...
    case SimpleLoop(s, i, ReduceElem(y)) =>
      syms(y) // could also return zero value
    case SimpleLoop(s, i, ArrayIfElem(c, y)) => Nil
    case SimpleLoop(s, i, ReduceIfElem(c, y)) =>
      syms(y) // could also return zero value
      ...
      trait Prog extends DSL {
        def test(x: Rep[Int]) = {
          ...
          a.update(
            40,
            40
          ) // error: not clear which object is mutated (vector1 or vector2)
      ...
      trait Prog extends DSL with LiftVariables {
        def test(x: Rep[Int]) = {
          ...
          for (i <- 0 until b.length) { // this is also a curious case: range creation must not be reflected
            ...
            a =
              b // error: here we learn that reads on a would need to be serialized with b but it's too late...
          }
        }
      }
      new Prog with Impl
    ...
    withOutFile(prefix + "mutation4b") {
      ...
      trait Prog extends DSL {
        def test(x: Rep[Int]) = {
          ...
          val b1 = b.clone
          val b2 =
            b1.mutable // error: internal arrays are *still* mutable, despite shallow clone

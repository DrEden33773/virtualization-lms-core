...
trait AbstractTransformer {
  ...
  def reflectBlock[A](xs: Block[A]): Exp[A] =
    sys.error("reflectBlock not supported by context-free transformers")
  ...
  def apply[X, Y, A](f: (X, Y) => Exp[A]): (X, Y) => Exp[A] = (z1: X, z2: Y) =>
    apply(f(z1, z2))
  def apply[X, Y, Z, A](f: (X, Y, Z) => Exp[A]): (X, Y, Z) => Exp[A] =
    (z1: X, z2: Y, z3: Z) => apply(f(z1, z2, z3))
  def apply[W, X, Y, Z, A](f: (W, X, Y, Z) => Exp[A]): (W, X, Y, Z) => Exp[A] =
    (z1: W, z2: X, z3: Y, z4: Z) => apply(f(z1, z2, z3, z4))
  def apply[V, W, X, Y, Z, A](
      f: (V, W, X, Y, Z) => Exp[A]
  ): (V, W, X, Y, Z) => Exp[A] = (z1: V, z2: W, z3: X, z4: Y, z5: Z) =>
    apply(f(z1, z2, z3, z4, z5))
  // def apply[A](xs: Summary): Summary = xs //TODO
  def onlySyms[A](xs: List[Sym[A]]): List[Sym[A]] =
    xs map (e => apply(e)) collect { case e: Sym[A] => e }
...
trait Transforming extends Expressions with Blocks with OverloadHack {
  ...
  class SubstTransformer
      extends /*AbstractSubstTransformer*/ AbstractTransformer {
    val IR: self.type = self
  ...
  /** Used to pass a `Typ[A]` when `Typ[B]` is expected, either because the
    * Scala compiler can't figure out they are the same (e.g. in GADTs) or
    * because we can't obtain the correct instance of `Typ[B]`
    */
  def mtype[A, B](m: Typ[A]): Typ[B] = m.asInstanceOf[Typ[
    B
  ]] // hack: need to pass explicit manifest during mirroring
  def mpos(s: List[SourceContext]): SourceContext = if (s.nonEmpty) s.head
  else
    implicitly[
      SourceContext
    ] // hack: got list of pos but need to pass single pos to mirror
  ...
  def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
      pos: SourceContext
  ): Exp[A] = mirrorDef(e, f)

  def mirrorDef[A: Typ](e: Def[A], f: Transformer)(implicit
      pos: SourceContext
  ): Def[A] = sys.error("don't know how to mirror " + e)

  def mirrorFatDef[A: Typ](e: Def[A], f: Transformer)(implicit
      pos: SourceContext
  ): Def[A] = sys.error("don't know how to mirror " + e) // hm...

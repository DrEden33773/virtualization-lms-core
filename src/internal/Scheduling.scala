...
trait Scheduling {
  ...
  protected def scheduleDepsWithIndex(
      syms: List[Sym[Any]],
    ...
    val sortedSet = new java.util.TreeSet[(Stm, Int)](
      new java.util.Comparator[(Stm, Int)] {
        def compare(a: (Stm, Int), b: (Stm, Int)) =
    ...
    val iter =
      sortedSet.iterator // return stms in the original order given by 'scope'
    while (iter.hasNext) {
      res ::= iter.next._1
    }
    res
  ...
  protected def buildScopeIndex(
    ...
    for (stm <- scope) {
      for (s <- stm.lhs)
        cache.put(s, (stm, idx)) // remember the original order of the stms
      idx += 1
    }
    cache
  }
  ...
  def getSchedule(
    ...
    val xx = GraphUtil.stronglyConnectedComponents[Stm](
      scheduleDepsWithIndex(syms(result), scopeIndex),
      t => scheduleDepsWithIndex(syms(t.rhs), scopeIndex)
    )
  ...
  def getScheduleM(
    ...
    def mysyms(st: Any) = {
      ...
      assert(
        syms(st).toSet == db.map(_._1).toSet,
        "different list of syms: " + syms(st) + "!=" + db + " for " + st
      )
    ...
    GraphUtil
      .stronglyConnectedComponents[Stm](
        scheduleDepsWithIndex(mysyms(result), scopeIndex),
        t => scheduleDepsWithIndex(mysyms(t.rhs), scopeIndex)
      )
      .flatten
      .reverse
  }
  ...
  def getFatDependentStuff(scope: List[Stm])(sts: List[Sym[Any]]): List[Stm] = {
    ...
    def putDef(
        map: IdentityHashMap[Sym[Any], List[Stm]],
        s: Sym[Any],
        d: Stm
    ): Unit = {
      var res = map.get(s) // map(s)
    ...
    def putDefSet(
        map: IdentityHashMap[Sym[Any], Set[Stm]],
        s: Sym[Any],
        d: Stm
    ): Unit = {
    ...
    for (d <- scope) {
      ...
      tunnelSyms(d.rhs).foreach(st =>
        putDef(boundSymsCache, st, d)
      ) // treat tunnel like bound
    }
    ...
    def getDepStuff(st: Sym[Any]) = {
      ...
      def uses(s: Sym[Any]): List[Stm] = if (seen(s)) Nil
      else {
        // seen += s
        lhsCache.getOrElse(s, Nil) ::: symsCache.getOrElse(
          s,
          Nil
        ) filterNot (boundSymsCache.getOrElse(st, Nil) contains _)
      }
      GraphUtil
        .stronglyConnectedComponents[Stm](
          uses(st),

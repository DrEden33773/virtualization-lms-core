...
trait CodeMotion extends Scheduling {
  ...
  def getExactScope[A](
    ...
    val f1 = g1.flatMap { t => syms(t.rhs) } flatMap { s =>
      h1 filter (_.lhs contains s)
    } // fringe: 1 step from g1

    val e2 = getScheduleM(e1)(
      result,
      false,
      true
    ) // (shallow|hot)*  no cold ref on path

    val e3 = getScheduleM(e1)(
      result,
      true,
      false
    ) // (shallow|cold)* no hot ref on path

    val f2 = f1 filterNot (e3 contains _) // fringe restricted to: any* hot any*

    val h2 = getScheduleM(e1)(
      f2.flatMap(_.lhs),
      false,
      true
    ...
    val loopsNotInIfs =
      e2 filterNot (e3 contains _) // (shallow|hot)* hot (shallow|hot)*   <---- a hot ref on all paths!
    val reachFromTopLoops = getSchedule(e1)(loopsNotInIfs, false)

    val f3 =
      f1 filter (reachFromTopLoops contains _) // fringe restricted to: (shallow|hot)* hot any*
    val h3 = getScheduleM(e1)(
      f3.flatMap(_.lhs),
      false,
      true
    ) // anything that depends non-cold on it...
    ...
    val levelScope = e1.filter(z =>
      (shouldOutside contains z) && !(g1 contains z)
    ) // shallow (but with the ordering of deep!!) and minus bound
    ...
    result foreach {
      case LocalDef(TP(_, Reify(x, u, effects))) =>
        val observable =
          if (addControlDeps) effects.filterNot(controlDep) else effects
        val acteffects =
          levelScope.flatMap(_.lhs) filter (observable contains _)
        if (observable.toSet != acteffects.toSet) {
          val actual = levelScope.filter(_.lhs exists (observable contains _))
          val expected = observable.map(d => /*fatten*/ (findDefinition(
              d.asInstanceOf[Sym[Any]]
            ).get)
          )
          ...
          missing.foreach { d =>
            ...
            printfn(
              "    " + d + " <-- inDeep: " + inDeep + ", inShallow: " + inShallow + ", inDep: " + inDep
            )

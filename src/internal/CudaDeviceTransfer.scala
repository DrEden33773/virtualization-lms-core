...
trait CudaDeviceTransfer extends AbstractDeviceTransfer {
  ...
  def emitSendSlave(tp: Typ[_]): (String, String) = {
    if (isPrimitiveType(tp)) {
      ...
      val signature = "%s sendCuda_%s(%s sym)".format(
        remap(tp),
      ...
      throw new GenerationFailedException(
        "CudaDeviceTransfer: Unknown type " + tp.toString
      )
    }
  }
  ...
  def emitRecvSlave(tp: Typ[_]): (String, String) = {
    if (isPrimitiveType(tp)) {
      val out = new StringBuilder
      val signature = "%s recvCuda_%s(%s *sym_dev)".format(
        remap(tp),
        mangledName(remap(tp)),
        remap(tp)
      )
      ...
      out.append(
        "\tDeliteCudaMallocHost((void**)&hostPtr,sizeof(%s));\n".format(
          remap(tp)
        )
      )
      out.append(
        "\tDeliteCudaMemcpyDtoHAsync(hostPtr, sym_dev, sizeof(%s));\n".format(
          remap(tp)
        )
      )
  ...
  def emitSendUpdateSlave(tp: Typ[_]): (String, String) = {
    if (isPrimitiveType(tp)) {
      val out = new StringBuilder
      val signature = "void sendUpdateCuda_%s(%s sym)".format(
        mangledName(remap(tp)),
        remap(tp)
      )
      out.append(signature + " {\n")
      ...
      val out = new StringBuilder
      val signature = "void sendUpdateCuda_%s(%s *sym_dev, %s *sym)".format(
        mangledName(remap(tp)),
        remap(tp),
        remapHost(tp)
      )
  ...
  def emitRecvUpdateSlave(tp: Typ[_]): (String, String) = {
    if (isPrimitiveType(tp)) {
      val out = new StringBuilder
      val signature = "void recvUpdateCuda_%s(%s sym)".format(
        mangledName(remap(tp)),
        remap(tp)
      )
      out.append(signature + " {\n")
      ...
      val out = new StringBuilder
      val signature = "void recvUpdateCuda_%s(%s *sym_dev, %s *sym)".format(
        mangledName(remap(tp)),
        remap(tp),
        remapHost(tp)
      )

...
trait CppHostTransfer extends AbstractHostTransfer {
  ...
  def emitSend(tp: Typ[_], peer: Targets.Value): (String, String) = {
    if (peer == Targets.JVM) {
      if (remap(tp) == "string") {
        ...
        val signature = "jobject sendCPPtoJVM_%s(JNIEnv *env, %s sym)".format(
          mangledName(remapHost(tp)),
          remap(tp)
        )
        ...
        val out = new StringBuilder
        val signature = "%s sendCPPtoJVM_%s(JNIEnv *env, %s sym)".format(
          JNIType(tp),
          mangledName(remapHost(tp)),
          remap(tp)
        )
        ...
        throw new GenerationFailedException(
          "CppHostTransfer: Unknown type " + tp.toString
        )
      }
    } else if (peer == Targets.Cpp) {
      throw new GenerationFailedException(
        "CppHostTransfer: Unknown peer " + peer.toString
      )
    } else {
      throw new GenerationFailedException(
        "CppHostTransfer: Unknown peer " + peer.toString
  ...
  def emitRecv(tp: Typ[_], peer: Targets.Value): (String, String) = {
    if (peer == Targets.JVM) {
      if (remap(tp) == "string") {
        val out = new StringBuilder
        val signature = "%s recvCPPfromJVM_%s(JNIEnv *env, jobject obj)".format(
          remapHost(tp),
          mangledName(remapHost(tp))
        )
        out.append(signature + " {\n")
        ...
        val out = new StringBuilder
        val signature = "%s recvCPPfromJVM_%s(JNIEnv *env, %s sym)".format(
          remap(tp),
          mangledName(remapHost(tp)),
          JNIType(tp)
        )
        ...
        val signature = "%s *recvCPPfromJVM_%s(JNIEnv *env, jobject obj)"
          .format(remapHost(tp), mangledName(remapHost(tp)))
        out.append(signature + " {\n")
        out.append(
  ...
  def emitSendView(tp: Typ[_], peer: Targets.Value): (String, String) = {
    if (peer == Targets.JVM) {
      if (remap(tp) == "string") {
        val out = new StringBuilder
        val signature = "jobject sendViewCPPtoJVM_%s(JNIEnv *env, %s sym)"
          .format(mangledName(remapHost(tp)), remapHost(tp))
        out.append(signature + " {\n")
        out.append("\tassert(false);\n")
        out.append("\treturn NULL;\n")
        out.append("}\n")
        ...
        val out = new StringBuilder
        val signature = "%s sendViewCPPtoJVM_%s(JNIEnv *env, %s sym)".format(
          JNIType(tp),
          mangledName(remapHost(tp)),
          remap(tp)
        )
  ...
  def emitRecvView(tp: Typ[_], peer: Targets.Value): (String, String) = {
    if (peer == Targets.JVM) {
      if (remap(tp) == "string") {
        val out = new StringBuilder
        val signature = "%s recvViewCPPfromJVM_%s(JNIEnv *env, jobject *obj)"
          .format(remapHost(tp), mangledName(remapHost(tp)))
        out.append(signature + " {\n")
        out.append("\tassert(false);\n")
        out.append("\treturn NULL;\n")
        ...
        val out = new StringBuilder
        val signature = "%s recvViewCPPfromJVM_%s(JNIEnv *env, %s sym)".format(
          remap(tp),
          mangledName(remapHost(tp)),
          JNIType(tp)
        )
  ...
  def emitSendUpdate(tp: Typ[_], peer: Targets.Value): (String, String) = {
    if (peer == Targets.JVM) {
      if (isPrimitiveType(tp)) {
        val out = new StringBuilder
        val signature = "void sendUpdateCPPtoJVM_%s(JNIEnv *env, %s sym)"
          .format(mangledName(remapHost(tp)), remap(tp))
        out.append(signature + " {\n")
        out.append("\tassert(false);\n")
        out.append("}\n")
        (signature + ";\n", out.toString)
        ...
        val out = new StringBuilder
        val signature =
          "void sendUpdateCPPtoJVM_%s(JNIEnv *env, jobject obj, %s *sym)"
            .format(mangledName(remapHost(tp)), remapHost(tp))
        out.append(signature + " {\n")
        out.append("\tassert(false);\n")
        out.append("}\n")
        ...
        val out = new StringBuilder
        val signature =
          "void sendUpdateCPPtoJVM_%s(JNIEnv *env, jobject obj, %s sym)".format(
            mangledName(remapHost(tp)),
            remapHost(tp)
          )
        out.append(signature + " {\n")
  ...
  def emitRecvUpdate(tp: Typ[_], peer: Targets.Value): (String, String) = {
    if (peer == Targets.JVM) {
      if (remap(tp) == "string") {
        val out = new StringBuilder
        val signature = "void recvUpdateCPPfromJVM_%s(JNIEnv *env, %s sym)"
          .format(mangledName(remapHost(tp)), remap(tp))
        out.append(signature + " {\n")
        out.append("\tassert(false);\n")
        out.append("}\n")
        ...
        val out = new StringBuilder
        val signature =
          "void recvUpdateCPPfromJVM_%s(JNIEnv *env, jobject obj, %s *sym)"
            .format(mangledName(remapHost(tp)), remapHost(tp))
        out.append(signature + " {\n")
        out.append("\tassert(false);\n")
        out.append("}\n")
  ...
  def JNITypeDescriptor(tp: String): String = tp match {
    ...
    case "Float"   => "F"
    case "Double"  => "D"
    case array if array.startsWith("Array[") =>
      "[" + JNITypeDescriptor(array.slice(6, array.length - 1))
    case _ => { // all other types are objects
      var objectType = tp.replace('.', '/')
      if (objectType.indexOf('[') != -1)
        objectType = objectType.substring(0, objectType.indexOf('[')) // erasure

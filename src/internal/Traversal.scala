...
//  var levelScope: List[TP[Any]] = Nil
  ...
  override def availableDefs =
    if (innerScope ne null) innerScope else initialDefs
...
//    innerScope = saveInner
  ...
  def getStronglySortedSchedule2(
    ...
    val inner =
      scope diff level // TODO: restrict to things referenced by functions (not ifs) ?

    var recursive: List[Sym[Any]] = Nil

    var xx = GraphUtil.stronglyConnectedComponents[Stm](
      scheduleDepsWithIndex(allSyms(result), scopeIndex),
    ...
    xx = GraphUtil.stronglyConnectedComponents[Stm](
      scheduleDepsWithIndex(allSyms(result) ++ allSyms(recursive), scopeIndex),
      t => scheduleDepsWithIndex(allSyms(t.rhs), scopeIndex)
    )
    xx.foreach { xs =>
      if (xs.length > 1 && (xs intersect level).nonEmpty) {
        ...
        printerr(
          "error: recursive schedule did not go away for result " + result + ": " + xs
        )
      }
    }
  ...
  def focusExactScopeSubGraph[A](
      result: List[Exp[Any]]
  )(body: List[Stm] => A): A = {
    val availDefs =
      availableDefs // getStronglySortedSchedule(availableDefs)(result) // resolve anti-dependencies (may still be recursive -- not sure whether that's a problem)
    val levelScope = getExactScope(availDefs)(result)
    val (levelScope2, recursive) = getStronglySortedSchedule2(
      availDefs,
      levelScope,
      result
    ) // resolve anti-dependencies and recursive declarations

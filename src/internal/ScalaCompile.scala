...
trait ScalaCompile extends Expressions {
  ...
  def setupCompiler() = {
    ...
    settings.classpath.value = this.getClass.getClassLoader match {
      case ctx: java.net.URLClassLoader =>
        ctx.getURLs.map(_.getPath).mkString(pathSeparator)
    ...
    reporter =
      new ConsoleReporter(settings, null, new PrintWriter(System.out)) // writer
  ...
  def compile[A, B](
    ...
    run.compileSources(
      List(new util.BatchSourceFile("<stdin>", source.toString))
    )
    ...
    val loader =
      new AbstractFileClassLoader(fileSystem, this.getClass.getClassLoader)

    val cls: Class[_] = loader.loadClass(className)
    val cons = cls.getConstructor(staticData.map(_._1.tp.runtimeClass): _*)

    val obj: A => B = cons
      .newInstance(staticData.map(_._2.asInstanceOf[AnyRef]): _*)
      .asInstanceOf[A => B]
    obj
  }
}

...
trait CLikeCodegen extends GenericCodegen {
  ...
  def mangledName(name: String) = name
    .replaceAll("\\s", "")
    .map(c => if (!c.isDigit && !c.isLetter) '_' else c)
  ...
  def emitValDef(sym: String, tpe: Typ[_], rhs: String): Unit = {
    if (remap(tpe) != "void")
      stream.println(remap(tpe) + " " + sym + " = " + rhs + ";")
  }
  ...
  override def remap[A](m: Typ[A]): String = m match {
    case VariableTyp(tp) =>
      remap(tp)
    case _
        if m.runtimeClass == classOf[List[
          _
        ]] => // Use case: Delite Foreach sync list
      ...
      m.toString match {
        ...
        case _ =>
          throw new GenerationFailedException(
            "CLikeGen: remap(m) : Type %s cannot be remapped.".format(
              m.toString
            )
          )
      }
  ...
  override def emitKernelHeader(
    ...
    def kernelSignature: String = {
      ...
      if (resultIsVar) {
        if (cppMemMgr == "refcnt")
          out.append(
            wrapSharedPtr(hostTarget + "Ref" + unwrapSharedPtr(resultType))
          )
        else
      ...
      out.append(
        vals
          .map(p => remap(p.tp) + " " + addRef(p.tp) + quote(p))
          .mkString(", ")
      )
        ...
        if (cppMemMgr == "refcnt")
          out.append(
            vars
              .map(v =>
                wrapSharedPtr(
                  hostTarget + "Ref" + unwrapSharedPtr(remap(v.tp))
                ) + " " + quote(v)
              )
              .mkString(",")
          )
          ...
          out.append(
            vars
              .map(v =>
                hostTarget + "Ref" + remap(v.tp) + addRef() + " " + quote(v)
              )
              .mkString(",")
          )
      }
  ...
  def isPrimitiveType(tpe: String): Boolean = {
    tpe match {
      case "bool" | "int8_t" | "uint16_t" | "int16_t" | "int32_t" | "int64_t" |
          "float" | "double" =>
        true
      case _ => false
    }

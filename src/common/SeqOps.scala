...
trait SeqOps extends Variables {
  ...
  def seq_apply[T: Typ](x: Rep[Seq[T]], n: Rep[Int])(implicit
      pos: SourceContext
  ): Rep[T]
...
trait SeqOpsExp extends SeqOps with PrimitiveOps with EffectExp {
  ...
  def seq_new[A: Typ](xs: Seq[Rep[A]])(implicit pos: SourceContext) = SeqNew(
    xs.toList
  )
  def seq_apply[T: Typ](x: Exp[Seq[T]], n: Exp[Int])(implicit
      pos: SourceContext
  ): Exp[T] = SeqApply(x, n)
  def seq_length[T: Typ](a: Exp[Seq[T]])(implicit
      pos: SourceContext
  ): Exp[Int] = SeqLength(a)

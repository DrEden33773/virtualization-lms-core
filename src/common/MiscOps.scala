...
trait MiscOps extends Base with PrimitiveOps with StringOps {
  ...
  /** Other things that need to get lifted like exit, there should be a better
    * way to do this
    */
  ...
  def exit(status: Int)(implicit pos: SourceContext): Rep[Unit] = exit(
    unit(status)
  )
...
trait MiscOpsExp
  ...
  def print(x: Exp[Any])(implicit pos: SourceContext) = reflectEffect(
    Print(x)
  ) // TODO: simple effect
  def println(x: Exp[Any])(implicit pos: SourceContext) = reflectEffect(
    PrintLn(x)
  ) // TODO: simple effect
  def printf(f: String, x: Rep[Any]*)(implicit pos: SourceContext): Rep[Unit] =
    reflectEffect(PrintF(f, x.toList))
  def exit(s: Exp[Int])(implicit pos: SourceContext) = reflectEffect(Exit(s))
  def error(s: Exp[String])(implicit pos: SourceContext) = reflectEffect(
    Error(s)
  )
  def returnL(x: Exp[Any])(implicit pos: SourceContext) = {
    printlog(
      "warning: staged return statements are unlikely to work because the surrounding source method does not exist in the generated code."
    )
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case Reflect(Error(x), u, es) =>
      reflectMirrored(Reflect(Error(f(x)), mapOver(f, u), f(es)))(mtyp1[A], pos)
    case Reflect(Print(x), u, es) =>
      reflectMirrored(Reflect(Print(f(x)), mapOver(f, u), f(es)))(mtyp1[A], pos)
    case Reflect(PrintLn(x), u, es) =>
      reflectMirrored(Reflect(PrintLn(f(x)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(PrintF(fm, x), u, es) =>
      reflectMirrored(Reflect(PrintF(fm, f(x)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(Exit(x), u, es) =>
      reflectMirrored(Reflect(Exit(f(x)), mapOver(f, u), f(es)))(mtyp1[A], pos)
    case Reflect(Return(x), u, es) =>
      reflectMirrored(Reflect(Return(f(x)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
...
trait CGenMiscOps extends CGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case Print(s) =>
      stream.println("printf(\"" + format(s) + "\"," + quoteRawString(s) + ");")

...
import scala.lms.internal.{
  GenericNestedCodegen,
  GenericFatCodegen,
  GenerationFailedException
}

trait SplitEffectsExpFat
    extends IfThenElseFatExp
    with WhileExp
    with PreviousIterationDummyExp {
  thisIR: BooleanOpsExp with EqualExpBridge =>
  ...
  override def reflectEffectInternal[A: Typ](x: Def[A], u: Summary)(implicit
      pos: SourceContext
  ): Exp[A] = x match {
    case IfThenElse(cond, thenp, elsep) =>
      ...
      for (s <- affected)
        super.reflectEffectInternal(
          IfThenElse(cond, projectB(thenp, List(s)), projectB(elsep, List(s))),
          projectS(u, List(s))
        )

      if (u.maySimple)
        super.reflectEffectInternal(
          IfThenElse(cond, projectSimpleB(thenp), projectSimpleB(elsep)),
          projectSimpleS(u)
        )

      super.reflectEffectInternal(
        IfThenElse(cond, projectPureB(thenp), projectPureB(elsep)),
        projectPureS(u)
      )
    ...
    case While(cond, body) =>
      ...
      for (s <- affected) {
        ...
        def xtract(b: Block[Any]) = b match {
          case Block(Def(Reify(_, _, es: List[Sym[Any]]))) =>
            es map (e => findDefinition(e)) collect {
              case Some(t @ TP(s1, Reflect(PreviousIteration(_), u, _)))
                  if mayWrite(u, List(s)) =>
                t
            }
        }
        ...
        def xform(x: Stm) = x match {
          case TP(s1, Reflect(PreviousIteration(k), u, es))
              if (pvss contains s1) =>
            // println("replace " + s1)
            TP(s1, Reflect(PreviousIteration(k), u, es :+ loopSym))
          case t => t
        }
        (thisIR: EmbeddedControls).__assign(
          globalDefs,
          globalDefs map xform
        ) // FIXME: SI-6100
        (thisIR: EmbeddedControls).__assign(
          localDefs,
          localDefs map xform
      ...
      if (u.maySimple)
        super.reflectEffectInternal(
          While(projectPureWithSimpleB(cond), projectSimpleB(body)),
          projectSimpleS(u)
        )
  ...
  def projectL(a: List[Sym[Any]], s: List[Sym[Any]]): List[Sym[Any]] =
    a filter (s contains _)
  def projectS(u: Summary, s: List[Sym[Any]]): Summary =
    Pure().copy(
      mayRead = projectL(u.mayRead, s),
      mstRead = projectL(u.mstRead, s),
      mayWrite = projectL(u.mayWrite, s),
      mstWrite = projectL(u.mstWrite, s)
    )
    ...
    case Block(Def(Reify(x, u, es))) =>
      Block(
        Reify(
          Const(),
          projectSimpleS(u),
          calculateDependencies(es, Simple(), false)
        )
      )
    case _ => Block(Const(()))
  }
  ...
  def projectPureWithB[A: Typ](b: Block[A], s: List[Sym[Any]]): Block[A] = {
    (projectPureB(b), projectB(b, s)) match {
      case (Block(x), Block(Def(Reify(Const(()), u, es)))) =>
        Block(Reify(x, u, es))
      case (a, _) => a
    }
  }
...
// REMOVE
  ...
  case class SimpleFatWhile(cond: Block[Boolean], body: List[Block[Any]])
      extends FatDef {
  ...
  override def symsFreq(e: Any): List[(Sym[Any], Double)] = e match {
    case x @ SimpleFatWhile(c, b) =>
      freqHot(c) ++ freqHot(b) ++ freqNormal(x.extradeps)
...
trait PreviousIterationDummyExp extends BaseFatExp with EffectExp {
  ...
  case class SimpleFatPrevious(k: Exp[Unit], extra: List[Exp[Any]])
      extends FatDef

  def reflectPreviousDummy(k: Exp[Unit], u: Summary) =
    reflectEffect(PreviousIteration(k), u)
...
trait BaseGenSplitEffects extends BaseGenIfThenElseFat with GenericFatCodegen {
  ...
  override def fatten(e: Stm): Stm = e match {
    case TP(s, d @ While(c, b)) =>
      TTP(List(s), List(d), SimpleFatWhile(c, List(b)))
  ...
  override def fattenAll(e: List[Stm]): List[Stm] = {
    ...
    val m = e2 groupBy {
      ...
      case t @ TTP(
            lhs,
            mhs,
            p @ SimpleFatWhile(Block(Def(Reify(c, _, _))), b)
          ) =>
        (c, "while")
    ...
    val g1 = m map {
      case ((c: Exp[Boolean], "if"), ifs: List[TTP]) =>
        TTP(
          ifs.flatMap(_.lhs),
          ifs.flatMap(_.mhs),
          SimpleFatIfThenElse(
            c,
            ifs.flatMap(_.rhs.asInstanceOf[SimpleFatIfThenElse].thenp),
      ...
      case ((c, "while"), wls: List[TTP]) =>
        val x = SimpleFatWhile(
          wls
            .map(_.rhs.asInstanceOf[SimpleFatWhile].cond)
            .apply(0), // FIXME: merge cond!!!
          wls.flatMap(_.rhs.asInstanceOf[SimpleFatWhile].body)
        )
        x.setExtraDeps(
          wls.flatMap(_.rhs.asInstanceOf[SimpleFatWhile].extradeps) diff wls
            .flatMap(_.lhs)
        )
        TTP(
          wls.flatMap(_.lhs),
          wls.flatMap(_.mhs), // TODO: merge cond blocks!
          x
        )
      case ((k: Exp[Nothing], "prev"), pvs: List[TTP]) =>
        TTP(
          ...
          SimpleFatPrevious(
            k,
            pvs.flatMap(_.rhs.asInstanceOf[SimpleFatPrevious].extra)
          )
        )
    }
  ...
  override def emitFatNode(symList: List[Sym[Any]], rhs: FatDef) = rhs match {
    case SimpleFatWhile(c, b) =>
      def quoteList[T](xs: List[Exp[T]]) = if (xs.length > 1)
        xs.map(quote).mkString("(", ",", ")")
      else xs.map(quote).mkString(",")
...
trait ScalaGenSplitEffects
    extends BaseGenSplitEffects
...
trait ScalaGenPreviousIterationDummy extends ScalaGenFat {
  ...
  override def emitFatNode(symList: List[Sym[Any]], rhs: FatDef) = rhs match {
    case SimpleFatPrevious(k, es) =>
      // dummy ...
      stream.println(
        "// dummy placeholder for previous iteration: " + symList + " = " + k + " / " + es
      )

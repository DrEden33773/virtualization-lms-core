...
/** The Base trait defines the type constructor Rep, which is the higher-kinded
  * type that allows for other DSL types to be polymorphically embedded.
  *
  * @since 0.1
  */
...
trait BaseExp extends Base with Expressions with Blocks with Transforming {
  ...
  protected def manifestTyp[T: Manifest]: Typ[T] = ManifestTyp(
...
trait EffectExp extends BaseExp with Effects {
  ...
  def mapOver(t: Transformer, u: Summary) = { // TODO: move to effects class?
    u.copy(
      mayRead = t.onlySyms(u.mayRead),
      mstRead = t.onlySyms(u.mstRead),
      mayWrite = t.onlySyms(u.mayWrite),
      mstWrite = t.onlySyms(u.mstWrite)
    )
  }
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case Reflect(x, u, es) =>
      reflectMirrored(mirrorDef(e, f).asInstanceOf[Reflect[A]])
    case Reify(x, u, es) => Reify(f(x), mapOver(f, u), f(es))

...
trait SetOps extends Base {
  ...
  def set_contains[A: Typ](s: Rep[Set[A]], i: Rep[A])(implicit
      pos: SourceContext
  ): Rep[Boolean]
  def set_add[A: Typ](s: Rep[Set[A]], i: Rep[A])(implicit
      pos: SourceContext
  ): Rep[Unit]
  def set_remove[A: Typ](s: Rep[Set[A]], i: Rep[A])(implicit
      pos: SourceContext
  ): Rep[Unit]
  ...
  def set_toseq[A: Typ](s: Rep[Set[A]])(implicit
      pos: SourceContext
  ): Rep[Seq[A]]
  def set_toarray[A: Typ](s: Rep[Set[A]])(implicit
      pos: SourceContext
  ): Rep[Array[A]]
}
...
trait SetOpsExp extends SetOps with ArrayOps with BooleanOps with EffectExp {
  ...
  def set_new[A: Typ](xs: Seq[Exp[A]])(implicit pos: SourceContext) =
    reflectMutable(SetNew(xs, typ[A]))
  def set_contains[A: Typ](s: Exp[Set[A]], i: Exp[A])(implicit
      pos: SourceContext
  ) = SetContains(s, i)
  def set_add[A: Typ](s: Exp[Set[A]], i: Exp[A])(implicit pos: SourceContext) =
    reflectWrite(s)(SetAdd(s, i))
  def set_remove[A: Typ](s: Exp[Set[A]], i: Exp[A])(implicit
      pos: SourceContext
  ) = reflectWrite(s)(SetRemove(s, i))
  def set_size[A: Typ](s: Exp[Set[A]])(implicit pos: SourceContext) = SetSize(s)
  def set_clear[A: Typ](s: Exp[Set[A]])(implicit pos: SourceContext) =
    reflectWrite(s)(SetClear(s))
  def set_toseq[A: Typ](s: Exp[Set[A]])(implicit pos: SourceContext) = SetToSeq(
    s
  )
  def set_toarray[A: Typ](s: Exp[Set[A]])(implicit pos: SourceContext) =
    SetToArray(s)
}
...
trait ScalaGenSetOps extends BaseGenSetOps with ScalaGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case SetNew(xs, mA) =>
      emitValDef(
        sym,
        src"collection.mutable.HashSet[$mA](" + (xs map { quote }).mkString(
          ","
        ) + ")"
      )

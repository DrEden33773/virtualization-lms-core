...
trait LivenessOpt extends NestedBlockTraversal {
  ...
  def calculateLivenessTransform(
      used: mutable.HashSet[Sym[Any]],
      t: Stm
  ): List[Stm] = t match {
    case TP(sym, Reify(x, u, es)) if used(sym) =>
      ...
      List(t)
    case TP(sym, rhs @ Reflect(_, u, es))
        if used(sym) ||
  ...
  override def focusBlock[A](result: Block[Any])(body: => A): A = {
    super.focusBlock(result) {
      ...
      val newInnerScope =
        innerScope.reverse.flatMap(calculateLivenessTransform(used, _).reverse)
    ...
    super.focusBlock(result) {
      ...
      if (saveDefUseMap ne null) {
        ...
        for ((k, vs) <- saveDefUseMap) {
          defUseMap += (k -> (saveDefUseMap(k) ++ defUseMap.getOrElse(
            k,
            Set()
          )))
        }

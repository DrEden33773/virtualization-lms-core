...
trait ArrayOps extends Variables {
  ...
  implicit def varToArrayOps[T: Typ](x: Var[Array[T]]) = new ArrayOpsCls(
    readVar(x)
  )
  ...
  class ArrayOpsCls[T: Typ](a: Rep[Array[T]]) {
    ...
    def foreach(block: Rep[T] => Rep[Unit])(implicit pos: SourceContext) =
      array_foreach(a, block)
  ...
  def array_apply[T: Typ](x: Rep[Array[T]], n: Rep[Int])(implicit
      pos: SourceContext
  ): Rep[T]
  def array_update[T: Typ](x: Rep[Array[T]], n: Rep[Int], y: Rep[T])(implicit
      pos: SourceContext
  ): Rep[Unit]
  def array_unsafe_update[T: Typ](x: Rep[Array[T]], n: Rep[Int], y: Rep[T])(
      implicit pos: SourceContext
  ): Rep[Unit]
  def array_length[T: Typ](x: Rep[Array[T]])(implicit
      pos: SourceContext
  ): Rep[Int]
  def array_foreach[T: Typ](x: Rep[Array[T]], block: Rep[T] => Rep[Unit])(
      implicit pos: SourceContext
  ): Rep[Unit]
  def array_copy[T: Typ](
      src: Rep[Array[T]],
      srcPos: Rep[Int],
      dest: Rep[Array[T]],
      destPos: Rep[Int],
  ...
  def array_sort[T: Typ](x: Rep[Array[T]])(implicit
      pos: SourceContext
  ): Rep[Array[T]]
  def array_map[A: Typ, B: Typ](
      a: Rep[Array[A]],
      f: Rep[A] => Rep[B]
  ): Rep[Array[B]]
  def array_toseq[A: Typ](a: Rep[Array[A]]): Rep[Seq[A]]
  def array_slice[A: Typ](
      a: Rep[Array[A]],
      start: Rep[Int],
      end: Rep[Int]
  ): Rep[Array[A]]
...
trait ArrayOpsExp extends ArrayOps with EffectExp with VariablesExp {
  ...
  case class ArrayUpdate[T: Typ](a: Exp[Array[T]], n: Exp[Int], y: Exp[T])
      extends Def[Unit] {
    def m = typ[T]
  }
  ...
  case class ArrayForeach[T](a: Exp[Array[T]], x: Sym[T], block: Block[Unit])
      extends Def[Unit]
  case class ArrayCopy[T: Typ](
      src: Exp[Array[T]],
      srcPos: Exp[Int],
      dest: Exp[Array[T]],
      destPos: Exp[Int],
  ...
  case class ArrayMap[A: Typ, B: Typ](
      a: Exp[Array[A]],
      x: Sym[A],
      block: Block[B]
  ) extends Def[Array[B]] {
  ...
  case class ArraySlice[A: Typ](a: Exp[Array[A]], s: Exp[Int], e: Exp[Int])
      extends Def[Array[A]]

  def array_obj_new[T: Typ](n: Exp[Int]) = reflectMutable(ArrayNew(n))
  def array_obj_fromseq[T: Typ](xs: Seq[Exp[T]]) =
    /*reflectMutable(*/ ArrayFromSeq(xs) /*)*/
  def array_apply[T: Typ](x: Exp[Array[T]], n: Exp[Int])(implicit
      pos: SourceContext
  ): Exp[T] = ArrayApply(x, n)
  def array_update[T: Typ](x: Exp[Array[T]], n: Exp[Int], y: Exp[T])(implicit
      pos: SourceContext
  ) = reflectWrite(x)(ArrayUpdate(x, n, y))
  def array_unsafe_update[T: Typ](x: Rep[Array[T]], n: Rep[Int], y: Rep[T])(
      implicit pos: SourceContext
  ) = ArrayUpdate(x, n, y)
  def array_length[T: Typ](a: Exp[Array[T]])(implicit
      pos: SourceContext
  ): Rep[Int] = toAtom(ArrayLength(a))
  def array_foreach[T: Typ](a: Exp[Array[T]], block: Exp[T] => Exp[Unit])(
      implicit pos: SourceContext
  ): Exp[Unit] = {
    val x = fresh[T]
  ...
  def array_copy[T: Typ](
    ...
    reflectWrite(dest)(ArrayCopy(src, srcPos, dest, destPos, len))
  def array_unsafe_copy[T: Typ](
      src: Exp[Array[T]],
      srcPos: Exp[Int],
      dest: Exp[Array[T]],
      destPos: Exp[Int],
      len: Exp[Int]
  )(implicit pos: SourceContext) = ArrayCopy(src, srcPos, dest, destPos, len)
  def array_sort[T: Typ](x: Exp[Array[T]])(implicit pos: SourceContext) =
    ArraySort(x)
  ...
  def array_slice[A: Typ](a: Rep[Array[A]], start: Rep[Int], end: Rep[Int]) =
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case e @ ArrayCopy(a, ap, d, dp, l) =>
      array_copy(f(a), f(ap), f(d), f(dp), f(l))(mtyp1[A], pos)
    case Reflect(e @ ArrayNew(n), u, es) =>
      reflectMirrored(Reflect(ArrayNew(f(n))(e.m), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(e @ ArrayLength(x), u, es) =>
      reflectMirrored(Reflect(ArrayLength(f(x))(e.m), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(e @ ArrayApply(l, r), u, es) =>
      reflectMirrored(
        Reflect(ArrayApply(f(l), f(r))(e.m), mapOver(f, u), f(es))
      )(mtyp1[A], pos)
    case Reflect(e @ ArraySort(x), u, es) =>
      reflectMirrored(Reflect(ArraySort(f(x))(e.m), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(e @ ArrayUpdate(l, i, r), u, es) =>
      reflectMirrored(
        Reflect(ArrayUpdate(f(l), f(i), f(r))(e.m), mapOver(f, u), f(es))
      )(mtyp1[A], pos)
    case Reflect(e @ ArrayCopy(a, ap, d, dp, l), u, es) =>
      reflectMirrored(
        Reflect(
          ArrayCopy(f(a), f(ap), f(d), f(dp), f(l))(e.m),
          mapOver(f, u),
          f(es)
        )
      )(mtyp1[A], pos)
...
trait ArrayOpsExpOpt extends ArrayOpsExp {
  ...
  /** @author
    ...
    */
  override def array_length[T: Typ](
      a: Exp[Array[T]]
  )(implicit pos: SourceContext): Rep[Int] = a match {
  ...
  override def array_apply[T: Typ](x: Exp[Array[T]], n: Exp[Int])(implicit
      pos: SourceContext
  ): Exp[T] = {
    if (context ne null) {
  ...
  override def array_update[T: Typ](x: Exp[Array[T]], n: Exp[Int], y: Exp[T])(
      implicit pos: SourceContext
  ) = {
    if (context ne null) {
      // find the last modification of array x
...
trait ScalaGenArrayOps extends BaseGenArrayOps with ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
        ...
        if (xs.size > ARRAY_LITERAL_MAX_SIZE) {
          ...
          val parseMethod =
            if (clazz == classOf[Int])
              ...
              clazz == classOf[Long] || clazz == classOf[
                Float
              ...
              throw new GenerationFailedException(
                s"Can't store an array of type $m in a file"
              )
          s"""scala.io.Source.fromFile("${symDataPath(
              sym
            )}").getLines.map($parseMethod).toArray"""
        } else {
          src"Array[$m]($xs)"
        }
      )
    ...
    case ArrayCopy(src, srcPos, dest, destPos, len) =>
      emitValDef(sym, src"System.arraycopy($src,$srcPos,$dest,$destPos,$len)")
    case a @ ArraySort(x) =>
      gen"""val $sym = {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      case ArrayLength(x) =>
        emitValDef(
          sym,
          src"sizeof($x)/sizeof(*$x)"
        ) // WARN: statically allocated elements only
      case ArrayApply(x, n)     => emitValDef(sym, src"$x[$n]")
      ...
      case ArraySlice(x, s, e) =>
        ...
        emitValDef(
          sym,
          src"({ size_t sz=sizeof(" + tp + ")*($e-$s); " + tp + "* r = (" + tp + "*)malloc(sz); memcpy(r,((" + tp + "*)$x)+$s,sz); r; })"
        )
      case _ => super.emitNode(sym, rhs)
    }
  }
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      ...
      case ArrayUpdate(x, n, y) =>
        stream.println(
          quote(x) + "->update(" + quote(n) + "," + quote(y) + ");"
        )
      case _ => super.emitNode(sym, rhs)
    }
  }

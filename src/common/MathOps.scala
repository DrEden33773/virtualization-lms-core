...
trait MathOps extends Base with PrimitiveOps {
  ...
  object Math {
    ...
    def atan2(x: Rep[Double], y: Rep[Double])(implicit pos: SourceContext) =
      math_atan2(x, y)
    def pow(x: Rep[Double], y: Rep[Double])(implicit pos: SourceContext) =
      math_pow(x, y)
    def abs[A: Typ: Numeric](x: Rep[A])(implicit pos: SourceContext) = math_abs(
      x
    )
    def max[A: Typ: Numeric](x: Rep[A], y: Rep[A])(implicit
        pos: SourceContext
    ) = math_max(x, y)
    def min[A: Typ: Numeric](x: Rep[A], y: Rep[A])(implicit
        pos: SourceContext
    ) = math_min(x, y)
    def Pi(implicit pos: SourceContext) =
  ...
  def math_atan2(x: Rep[Double], y: Rep[Double])(implicit
      pos: SourceContext
  ): Rep[Double]
  def math_pow(x: Rep[Double], y: Rep[Double])(implicit
      pos: SourceContext
  ): Rep[Double]
  def math_abs[A: Typ: Numeric](x: Rep[A])(implicit pos: SourceContext): Rep[A]
  def math_max[A: Typ: Numeric](x: Rep[A], y: Rep[A])(implicit
      pos: SourceContext
  ): Rep[A]
  def math_min[A: Typ: Numeric](x: Rep[A], y: Rep[A])(implicit
      pos: SourceContext
  ): Rep[A]
...
trait MathOpsExp extends MathOps with EffectExp {
  ...
  def math_atan2(x: Exp[Double], y: Exp[Double])(implicit pos: SourceContext) =
    MathAtan2(x, y)
  def math_pow(x: Exp[Double], y: Exp[Double])(implicit pos: SourceContext) =
    MathPow(x, y)
  def math_abs[A: Typ: Numeric](x: Exp[A])(implicit pos: SourceContext) =
    MathAbs(x)
  def math_max[A: Typ: Numeric](x: Exp[A], y: Exp[A])(implicit
      pos: SourceContext
  ) = MathMax(x, y)
  def math_min[A: Typ: Numeric](x: Exp[A], y: Exp[A])(implicit
      pos: SourceContext
  ) = MathMin(x, y)
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    e match {
      ...
      case Reflect(MathCeil(x), u, es) =>
        reflectMirrored(Reflect(MathCeil(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathFloor(x), u, es) =>
        reflectMirrored(Reflect(MathFloor(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathExp(x), u, es) =>
        reflectMirrored(Reflect(MathExp(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathPow(x, y), u, es) =>
        reflectMirrored(Reflect(MathPow(f(x), f(y)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathAbs(x), u, es) =>
        reflectMirrored(Reflect(MathAbs(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathSin(x), u, es) =>
        reflectMirrored(Reflect(MathSin(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathCos(x), u, es) =>
        reflectMirrored(Reflect(MathCos(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathAcos(x), u, es) =>
        reflectMirrored(Reflect(MathAcos(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathLog(x), u, es) =>
        reflectMirrored(Reflect(MathLog(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathSqrt(x), u, es) =>
        reflectMirrored(Reflect(MathSqrt(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathAtan2(x, y), u, es) =>
        reflectMirrored(Reflect(MathAtan2(f(x), f(y)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathMin(x, y), u, es) =>
        reflectMirrored(Reflect(MathMin(f(x), f(y)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathMax(x, y), u, es) =>
        reflectMirrored(Reflect(MathMax(f(x), f(y)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathLog10(x), u, es) =>
        reflectMirrored(Reflect(MathLog10(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathSinh(x), u, es) =>
        reflectMirrored(Reflect(MathSinh(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathAsin(x), u, es) =>
        reflectMirrored(Reflect(MathAsin(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathCosh(x), u, es) =>
        reflectMirrored(Reflect(MathCosh(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathTan(x), u, es) =>
        reflectMirrored(Reflect(MathTan(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
      case Reflect(MathTanh(x), u, es) =>
        reflectMirrored(Reflect(MathTanh(f(x)), mapOver(f, u), f(es)))(
          mtyp1[A],
          pos
        )
...
trait ScalaGenMathOpsApacheCommons extends ScalaGenMathOps {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) =
    rhs match { // TODO: use java.lang.Math etc...
      case MathCeil(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.ceil($x)")
      case MathFloor(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.floor($x)")
      case MathExp(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.exp($x)")
      case MathLog(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.log($x)")
      case MathSqrt(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.sqrt($x)")
      case MathSin(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.sin($x)")
      case MathCos(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.cos($x)")
      case MathAcos(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.acos($x)")
      case MathAtan(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.atan($x)")
      case MathAtan2(x, y) =>
        emitValDef(
          sym,
          src"org.apache.commons.math.util.FastMath.atan2($x, $y)"
        )
      case MathPow(x, y) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.pow($x,$y)")
      case MathAbs(x) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.abs($x)")
      case MathMax(x, y) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.max($x, $y)")
      case MathMin(x, y) =>
        emitValDef(sym, src"org.apache.commons.math.util.FastMath.min($x, $y)")
      case MathPi() =>
        emitValDef(sym, "org.apache.commons.math.util.FastMath.PI")
...
trait CLikeGenMathOps extends BaseGenMathOps with CLikeGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case MathMax(x, y) if (remap(sym.tp)) == "float" =>
      emitValDef(sym, src"fmax($x, $y)")
    case MathMax(x, y) if (remap(sym.tp)) == "double" =>
      emitValDef(sym, src"fmax($x, $y)")
    case MathMax(x, y) if (remap(sym.tp)) == "int" =>
      emitValDef(sym, src"max($x, $y)")
    case MathMin(x, y) if (remap(sym.tp)) == "float" =>
      emitValDef(sym, src"fmin($x, $y)")
    case MathMin(x, y) if (remap(sym.tp)) == "double" =>
      emitValDef(sym, src"fmin($x, $y)")
    case MathMin(x, y) if (remap(sym.tp)) == "int" =>
      emitValDef(sym, src"min($x, $y)")
    case _ => super.emitNode(sym, rhs)
  }
}

...
trait StaticDataExp extends EffectExp {
  ...
  override implicit def toAtom[T: Typ](d: Def[T])(implicit pos: SourceContext) =
    d match {
...
trait BaseGenStaticData extends GenericNestedCodegen {
  ...
  def getFreeDataExp[A](sym: Sym[A], rhs: Def[A]): List[(Sym[Any], Any)] =
    rhs match {
...
trait ScalaGenStaticData extends ScalaGenEffect with BaseGenStaticData {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case StaticData(x) =>
      emitValDef(
        sym,
        "p" + quote(sym) + " // static data: " + (x match {

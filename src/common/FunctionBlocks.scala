...
trait FunctionBlocksExp
   ...
   * BlockN definitions
   */
  implicit def lambdaToBlock0[R: Typ](f: () => Exp[R]) = Block0(
    reifyEffects(f())
  )
  ...
  implicit def lambdaToBlock2[T1: Typ, T2: Typ, R: Typ](
      f: (Exp[T1], Exp[T2]) => Exp[R]
  ) = {
    val (x1, x2) = (fresh[T1], fresh[T2])
    Block2(x1, x2, reifyEffects(f(x1, x2)))
  }
  case class Block2[T1: Typ, T2: Typ, R: Typ](
      blockArg1: Sym[T1],
      blockArg2: Sym[T2],
      blockRes: Block[R]
  )
  ...
  def transformBlockWithBound[A](
      t: ForwardTransformer { val IR: FunctionBlocksExp.this.type },
      f: Block[A],
      boundVars: List[(Exp[Any], Exp[Any])]
  ) = {
  ...
  implicit def transformerToBlockTransformer(t: ForwardTransformer {
    val IR: FunctionBlocksExp.this.type
  }) = new {
    def apply[R](x: Block0[R]): (() => Exp[R]) = { () =>
      t.reflectBlock(x.blockRes)
    }
    def apply[T1, R](x: Block1[T1, R]): Exp[T1] => Exp[R] = { a =>
      transformBlockWithBound(t, x.blockRes, List(x.blockArg1 -> a))
    }
    def apply[T1, T2, R](x: Block2[T1, T2, R]): (Exp[T1], Exp[T2]) => Exp[R] = {
      (a, b) =>
        transformBlockWithBound(
          t,
          x.blockRes,
          List(x.blockArg1 -> a, x.blockArg2 -> b)
        )
    }
  }
  ...
  def copyBlock1[T1: Typ, R: Typ](b: Block1[T1, R], t: Transformer) =
    Block1(t(b.blockArg1).asInstanceOf[Sym[T1]], t(b.blockRes))
  def copyBlock2[T1: Typ, T2: Typ, R: Typ](
      b: Block2[T1, T2, R],
      t: Transformer
  ) = Block2(
    t(b.blockArg1).asInstanceOf[Sym[T1]],
    t(b.blockArg2).asInstanceOf[Sym[T2]],
    t(b.blockRes)
  )
}

...
trait LiftEquals extends Base {
  ...
  def __equal[A: Typ, B: Typ](a: A, b: B): Rep[Boolean] =
    equals(unit(a), unit(b))
  def infix_!=[A: Typ, B: Typ](a: A, b: B): Rep[Boolean] =
...
trait Equal extends Base with Variables with OverloadHack {
  ...
  def __equal[A, B](a: Rep[A], b: Rep[B])(implicit
      o: Overloaded1,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ): Rep[Boolean] = equals(a, b)
  def __equal[A, B](a: Rep[A], b: Var[B])(implicit
      o: Overloaded2,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ): Rep[Boolean] = equals(a, b)
  def __equal[A, B](a: Var[A], b: Rep[B])(implicit
      o: Overloaded3,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ): Rep[Boolean] = equals(a, b)
  def __equal[A, B](a: Rep[A], b: B)(implicit
      o: Overloaded4,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ): Rep[Boolean] = equals(a, unit(b))
  def __equal[A, B](a: A, b: Rep[B])(implicit
      o: Overloaded5,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ): Rep[Boolean] = equals(unit(a), b)
  def __equal[A, B](a: Var[A], b: B)(implicit
      o: Overloaded6,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ): Rep[Boolean] = equals(a, unit(b))
  def __equal[A, B](a: A, b: Var[B])(implicit
      o: Overloaded7,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ): Rep[Boolean] = equals(unit(a), b)
  def __equal[A, B](a: Var[A], b: Var[B])(implicit
      o: Overloaded8,
      mA: Typ[A],
      mB: Typ[B],
      pos: SourceContext
  ...
  def equals[A: Typ, B: Typ](a: Rep[A], b: Rep[B])(implicit
      pos: SourceContext
  ): Rep[Boolean]
  def notequals[A: Typ, B: Typ](a: Rep[A], b: Rep[B])(implicit
      pos: SourceContext
  ): Rep[Boolean]
}
...
trait EqualExpBridge extends BaseExp with BooleanOpsExp {
  ...
  case class NotEqual[A: Typ, B: Typ](a: Exp[A], b: Exp[B])
      extends Def[Boolean] {
    def mA = typ[A]
    def mB = typ[B]
  }
  ...
  def equals[A: Typ, B: Typ](a: Rep[A], b: Rep[B])(implicit
      pos: SourceContext
  ): Rep[Boolean] = Equal(a, b)
  def notequals[A: Typ, B: Typ](a: Rep[A], b: Rep[B])(implicit
      pos: SourceContext
  ): Rep[Boolean] = NotEqual(a, b)
...
trait EqualExpBridgeOpt extends EqualExpBridge {
  override def equals[A: Typ, B: Typ](a: Rep[A], b: Rep[B])(implicit
      pos: SourceContext
  ...
  override def notequals[A: Typ, B: Typ](a: Rep[A], b: Rep[B])(implicit
      pos: SourceContext
  ): Rep[Boolean] = if (a == b) Const(false)
  else
    (a, b) match {
      case (Const(a), Const(b)) => Const(a != b)
...
trait CGenEqual extends CGenBase with CLikeGenEqual {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      case Equal(a, b)
          if (remap(a.tp) == "string" && remap(b.tp) == "string") =>
        emitValDef(sym, quote(a) + ".compare(" + quote(b) + ") == 0")
      case Equal(a, b)
          if (!isPrimitiveType(a.tp) && !isPrimitiveType(b.tp) && (remap(
            a.tp
          ) == remap(b.tp))) =>

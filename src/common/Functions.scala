...
trait Functions extends Base {
  ...
  def doLambda[A: Typ, B: Typ](fun: Rep[A] => Rep[B])(implicit
      pos: SourceContext
  ): Rep[A => B]
  implicit def fun[A: Typ, B: Typ](f: Rep[A] => Rep[B]): Rep[A => B] = doLambda(
    f
  )

  implicit def toLambdaOps[A: Typ, B: Typ](fun: Rep[A => B]) = new LambdaOps(
    fun
  )
  ...
  def doApply[A: Typ, B: Typ](fun: Rep[A => B], arg: Rep[A])(implicit
      pos: SourceContext
  ): Rep[B]
}
...
trait TupledFunctions extends Functions with TupleOps {
  implicit def fun[B: Typ](f: () => Rep[B]): Rep[Unit => B] =
    fun((t: Rep[Unit]) => f())
  implicit def fun[A1: Typ, A2: Typ, B: Typ](
      f: (Rep[A1], Rep[A2]) => Rep[B]
  ): Rep[((A1, A2)) => B] =
    fun((t: Rep[(A1, A2)]) => f(tuple2_get1(t), tuple2_get2(t)))
  implicit def fun[A1: Typ, A2: Typ, A3: Typ, B: Typ](
      f: (Rep[A1], Rep[A2], Rep[A3]) => Rep[B]
  ): Rep[((A1, A2, A3)) => B] =
    fun((t: Rep[(A1, A2, A3)]) =>
      f(tuple3_get1(t), tuple3_get2(t), tuple3_get3(t))
    )
  implicit def fun[A1: Typ, A2: Typ, A3: Typ, A4: Typ, B: Typ](
      f: (Rep[A1], Rep[A2], Rep[A3], Rep[A4]) => Rep[B]
  ): Rep[((A1, A2, A3, A4)) => B] =
    fun((t: Rep[(A1, A2, A3, A4)]) =>
      f(tuple4_get1(t), tuple4_get2(t), tuple4_get3(t), tuple4_get4(t))
    )
  implicit def fun[A1: Typ, A2: Typ, A3: Typ, A4: Typ, A5: Typ, B: Typ](
      f: (Rep[A1], Rep[A2], Rep[A3], Rep[A4], Rep[A5]) => Rep[B]
  ): Rep[((A1, A2, A3, A4, A5)) => B] =
    fun((t: Rep[(A1, A2, A3, A4, A5)]) =>
      f(
        tuple5_get1(t),
        tuple5_get2(t),
        tuple5_get3(t),
  ...
  class LambdaOps3[A1: Typ, A2: Typ, A3: Typ, B: Typ](
      f: Rep[((A1, A2, A3)) => B]
  ) {
    def apply(x1: Rep[A1], x2: Rep[A2], x3: Rep[A3]) = doApply(f, (x1, x2, x3))
    def apply(x: Rep[(A1, A2, A3)]): Rep[B] = doApply(f, x)
  }
  class LambdaOps4[A1: Typ, A2: Typ, A3: Typ, A4: Typ, B: Typ](
      f: Rep[((A1, A2, A3, A4)) => B]
  ) {
    def apply(x1: Rep[A1], x2: Rep[A2], x3: Rep[A3], x4: Rep[A4]) =
      doApply(f, (x1, x2, x3, x4))
    def apply(x: Rep[(A1, A2, A3, A4)]): Rep[B] = doApply(f, x)
  }
  class LambdaOps5[A1: Typ, A2: Typ, A3: Typ, A4: Typ, A5: Typ, B: Typ](
      f: Rep[((A1, A2, A3, A4, A5)) => B]
  ) {
    def apply(x1: Rep[A1], x2: Rep[A2], x3: Rep[A3], x4: Rep[A4], x5: Rep[A5]) =
      doApply(f, (x1, x2, x3, x4, x5))
    def apply(x: Rep[(A1, A2, A3, A4, A5)]): Rep[B] = doApply(f, x)
  }
  // implicit def toLambdaOpsAny[B:Typ](fun: Rep[Any => B]) =
  //  toLambdaOps(fun)
  implicit def toLambdaOps2[A1: Typ, A2: Typ, B: Typ](
      fun: Rep[((A1, A2)) => B]
  ) =
    new LambdaOps2(fun)
  implicit def toLambdaOps3[A1: Typ, A2: Typ, A3: Typ, B: Typ](
      fun: Rep[((A1, A2, A3)) => B]
  ) =
    new LambdaOps3(fun)
  implicit def toLambdaOps4[A1: Typ, A2: Typ, A3: Typ, A4: Typ, B: Typ](
      fun: Rep[((A1, A2, A3, A4)) => B]
  ) =
    new LambdaOps4(fun)
  implicit def toLambdaOps5[
      A1: Typ,
      A2: Typ,
      A3: Typ,
      A4: Typ,
...
trait FunctionsExp extends Functions with EffectExp {
  ...
  case class Lambda[A: Typ, B: Typ](f: Exp[A] => Exp[B], x: Exp[A], y: Block[B])
      extends Def[A => B] { def mA = typ[A]; def mB = typ[B] }
  case class Apply[A: Typ, B: Typ](f: Exp[A => B], arg: Exp[A]) extends Def[B] {
    def mA = typ[A]; def mB = typ[B]
  }
  ...
  override def doLambda[A: Typ, B: Typ](f: Exp[A] => Exp[B])(implicit
      pos: SourceContext
  ): Exp[A => B] =
    doLambdaDef(f)

  override def doApply[A: Typ, B: Typ](f: Exp[A => B], x: Exp[A])(implicit
      pos: SourceContext
  ): Exp[B] = {
    val x1 = unbox(x)
    f match {
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case e @ Lambda(g, x: Exp[Any], y: Block[b]) =>
      toAtom(Lambda(f(g), f(x), f(y))(e.mA, e.mB))(mtyp1[A], pos)
    case e @ Apply(g, arg) => doApply(f(g), f(arg))(e.mA, mtype(e.mB), pos)
    case Reflect(e @ Apply(g, arg), u, es) =>
      reflectMirrored(
        Reflect(Apply(f(g), f(arg))(e.mA, mtype(e.mB)), mapOver(f, u), f(es))
      )(mtyp1[A], pos)
    case _ => super.mirror(e, f)
  }).asInstanceOf[Exp[A]] // why??

  override def mirrorDef[A: Typ](e: Def[A], f: Transformer)(implicit
      pos: SourceContext
  ): Def[A] = (e match {
...
trait TupledFunctionsExp
  ...
  private def tupledTyp[T](m: Typ[T]): Boolean =
    m.runtimeClass.getName startsWith "scala.Tuple"
  private def tupledTypOf[T](m: Typ[T], arity: Int): Boolean =
    m.runtimeClass.getName == "scala.Tuple" + arity
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case e @ Lambda(g, UnboxedTuple(xs), y: Block[b]) =>
      toAtom(Lambda(f(g), UnboxedTuple(f(xs))(e.mA), f(y))(e.mA, e.mB))(
        mtyp1[A],
        implicitly[SourceContext]
      )
    case _ => super.mirror(e, f)
  }).asInstanceOf[Exp[A]]
}
...
trait FunctionsRecursiveExp extends FunctionsExp with ClosureCompare {
  ...
  override def doLambda[A: Typ, B: Typ](
      f: Exp[A] => Exp[B]
...
trait TupledFunctionsRecursiveExp
    extends FunctionsRecursiveExp
...
trait ScalaGenTupledFunctions
    extends ScalaGenFunctions
  ...
  def unwrapTupleStr[A](m: Typ[A]): String = {
    ...
    if (s.startsWith("scala.Tuple"))
      s.slice(s.indexOf("[") + 1, s.length - 1).filter(c => c != ' ')
    else remap(m)
  }
  ...
  override def remap[A](m: Typ[A]): String = m.toString match {
    case f if f.startsWith("scala.Function") =>
      ...
      "scala.Function" + size + "[" + targsUnboxed.mkString(
        ","
      ) + sep + res + "]"
    ...
    rhs match {
      case e @ Lambda(fun, x, y) =>
        throw new GenerationFailedException(
          "CudaGenFunctions: Lambda is not supported yet"
        )
...
trait OpenCLGenFunctions extends OpenCLGenEffect with BaseGenFunctions {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      case e @ Lambda(fun, x, y) =>
        throw new GenerationFailedException(
          "OpenCLGenFunctions: Lambda is not supported yet"
        )
      case Apply(fun, arg) =>
        emitValDef(sym, quote(fun) + "(" + quote(arg) + ")")
...
trait CGenFunctions extends CGenEffect with BaseGenFunctions {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case e @ Lambda(fun, x, y) =>
      ...
      val retTp =
        if (cppExplicitFunRet == "true")
          "function<" + retType + "(" + remap(x.tp) + ")>"
        else "auto"
      stream.println(
        retTp + " " + quote(sym) +
          " = [&](" + remap(x.tp) + " " + quote(x) + ") {"
...
trait CGenTupledFunctions
    extends CGenFunctions
    with GenericGenUnboxedTupleAccess {
  val IR: TupledFunctionsExp
  import IR._

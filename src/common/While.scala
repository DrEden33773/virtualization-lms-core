...
trait While extends Base {
  def __whileDo(cond: => Rep[Boolean], body: => Rep[Unit])(implicit
      pos: SourceContext
  ): Rep[Unit]
}
...
trait WhileExp extends While with BooleanOps with EffectExp {
  ...
  override def __whileDo(cond: => Exp[Boolean], body: => Rep[Unit])(implicit
      pos: SourceContext
  ) = {
...
trait WhileExpOptSpeculative extends WhileExp with PreviousIterationDummyExp {
  ...
  override def __whileDo(cond: => Exp[Boolean], body: => Rep[Unit])(implicit
    ...
    val c2 = reifyEffectsHere {
      reflectPreviousDummy(pb, ae1 /*.lastIteration*/ ); cond
    }
...
trait ScalaGenWhileOptSpeculative
    extends ScalaGenWhile
    with ScalaGenPreviousIterationDummy {

...
trait ImplicitOps extends Base {
  ...
  /** Implicit conversion from Rep[X] to Rep[Y]
    *
    * As long as a conversion is in scope, it will be invoked in the generated
    * scala code. Code-gen for other platforms should implement the conversions.
    */
  def implicit_convert[X, Y](x: Rep[X])(implicit
      c: X => Y,
      mX: Typ[X],
      mY: Typ[Y],
      pos: SourceContext
  ): Rep[Y] // = x.asInstanceOf[Rep[Y]
}

trait ImplicitOpsExp extends ImplicitOps with BaseExp {
  case class ImplicitConvert[X, Y](x: Exp[X])(implicit
      val mX: Typ[X],
      val mY: Typ[Y]
  ) extends Def[Y]

  def implicit_convert[X, Y](
      x: Exp[X]
  )(implicit c: X => Y, mX: Typ[X], mY: Typ[Y], pos: SourceContext): Rep[Y] = {
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case im @ ImplicitConvert(x) =>
      toAtom(ImplicitConvert(f(x))(im.mX, im.mY))(mtyp1[A], pos)

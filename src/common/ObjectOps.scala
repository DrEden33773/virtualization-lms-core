...
trait ObjectOps extends Variables with StringOps with OverloadHack {
  ...
  def infix_ToString(lhs: Rep[Any])(implicit pos: SourceContext) =
    object_tostring(lhs)
  def infix_unsafeImmutable[A: Typ](lhs: Rep[A])(implicit pos: SourceContext) =
    object_unsafe_immutable(lhs)
  def infix_unsafeMutable[A: Typ](lhs: Rep[A])(implicit pos: SourceContext) =
    object_unsafe_mutable(lhs)

  def object_tostring(lhs: Rep[Any])(implicit pos: SourceContext): Rep[String]
  def object_unsafe_immutable[A: Typ](lhs: Rep[A])(implicit
      pos: SourceContext
  ): Rep[A]
  def object_unsafe_mutable[A: Typ](lhs: Rep[A])(implicit
      pos: SourceContext
  ): Rep[A]
}
...
trait ObjectOpsExp extends ObjectOps with StringOpsExp with VariablesExp {
  ...
  def object_tostring(lhs: Exp[Any])(implicit pos: SourceContext) =
    ObjectToString(lhs)
  def object_unsafe_immutable[A: Typ](
      lhs: Exp[A]
  )(implicit pos: SourceContext) = lhs match {
  ...
  def object_unsafe_mutable[A: Typ](lhs: Exp[A])(implicit pos: SourceContext) =
    reflectMutable(ObjectUnsafeMutable(lhs))
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case e @ ObjectUnsafeImmutable(a) =>
      object_unsafe_immutable(f(a))(mtype(e.m), pos)
    case e @ ObjectToString(a) => object_tostring(f(a))
    case Reflect(e @ ObjectUnsafeImmutable(a), u, es) =>
      reflectMirrored(
        Reflect(ObjectUnsafeImmutable(f(a))(mtype(e.m)), mapOver(f, u), f(es))
      )(mtyp1[A], pos)
    case Reflect(e @ ObjectUnsafeMutable(a), u, es) =>
      reflectMirrored(
        Reflect(ObjectUnsafeMutable(f(a))(mtype(e.m)), mapOver(f, u), f(es))
      )(mtyp1[A], pos)
...
trait CLikeGenObjectOps extends CLikeGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case ObjectUnsafeImmutable(x) =>
      emitValDef(sym, src"$x; // unsafe immutable")
    case ObjectUnsafeMutable(x) => emitValDef(sym, src"$x; // unsafe mutable")
    case _                      => super.emitNode(sym, rhs)
  }
}

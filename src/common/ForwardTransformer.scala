...
trait ForwardTransformer
    ...
    case TP(sym, rhs) =>
      ...
      if (sym2 == sym) {
        ...
        if (recursive.contains(sym)) { // O(n) since recursive is a list!
          ...
          printerr(
            "warning: transformer already has a substitution " + sym + "->" + sym2 + " when encountering stm " + stm
          )
  ...
  def transformStm(stm: Stm): Exp[Any] =
    stm match { // override this to implement custom transform
      case TP(sym, rhs) =>
        /*
  ...
  def self_mirror[A](sym: Sym[A], rhs: Def[A]): Exp[A] = {
    try {
      mirror(rhs, self.asInstanceOf[Transformer])(
        mtype(sym.tp),
        mpos(sym.pos)
      ) // cast needed why?

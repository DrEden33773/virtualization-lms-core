...
trait SimplifyTransform extends internal.FatScheduling {
  ...
  def transformOne[A](s: Sym[A], x: Def[A], t: SubstTransformer): Exp[A] = {
    ...
    val y =
      try {
        ...
        if (ss != tss) {
          ...
          if (s2 == s) {
            printerr(
              "warning: mirroring of " + s + "=" + x + " syms " + ss.mkString(
                ","
              ) + " returned same object (expected t(syms) = " + tss.mkString(
                ","
              ) + ")"
            )
          }
          s2 match {
            case Def(x2) =>
              if (x.getClass == x2.getClass) {
                ...
                val ss2 = syms(x2)
                if (
                  ss2 != tss.filter(_.isInstanceOf[Sym[Any]])
                ) // should do filtering in def of tss above?
                  printerr(
                    "warning: mirroring of " + s + "=" + x + " syms " + ss
                      .mkString(
                        ","
                      ) + " returned " + s2 + "=" + x2 + " syms " + ss2
                      .mkString(",") + " (expected t(syms) = " + tss.mkString(
                      ","
                    ) + ")"
                  )
              }
              if (!(s2.tp <:< s.tp))
                printerr(
                  "warning: mirroring of " + s + "=" + x + " type " + s.tp + " returned " + s2 + "=" + x2 + " type " + s2.tp + " (not a subtype)"
                )
            case _ =>
              if (!(s2.tp <:< s.tp))
                printerr(
                  "warning: mirroring of " + s + "=" + x + " type " + s.tp + " returned " + s2 + " type " + s2.tp + " (not a subtype)"
                )
          }
          s2
        } else {
          printdbg(
            "skipping mirror operation " + s + "=" + x + " syms " + ss.mkString(
              ","
            ) + " subst " + t.subst.mkString(",")
          )
          s
        }
      } catch { // hack
    ...
    if (y != s) {
      if (
        y.isInstanceOf[Sym[Any]] && findDefinition(
          y.asInstanceOf[Sym[Any]]
        ).nonEmpty
      )
        printdbg(
          "--> replace " + s + "=" + x + " by " + y + "=" + findDefinition(
            y.asInstanceOf[Sym[Any]]
          ).get.defines(y.asInstanceOf[Sym[Any]]).get
        )
      else
    ...
    scope flatMap {
      case TP(sym, rhs) =>
        transformOne(sym, rhs, t) match {
          case s: Sym[Any] =>
            (Option(scopeIndex.get(s)) orElse findDefinition(
              s
            )).toList // check scope before target graph
          case _ => Nil
        }
      case TTP(lhs, mhs, SimpleFatIfThenElse(c, as, bs)) =>
        // alternate strategy: transform thin def, then fatten again (a little detour)
        printdbg(
          "need to transform rhs of fat if/then/else: " + lhs + ", if " + c + " then " + as + " else " + bs
        )
        val lhs2 = (lhs zip mhs)
          .map { case (s, r) => transformOne(s, r, t) }
          .distinct
        ...
        if (lhs != lhs2) {
          val missing =
            Nil // (lhs2.map(s => findDefinition(s).get) diff innerScope)
          printdbg(
            "lhs changed! will add to innerScope: " + missing.mkString(",")
          )
        ...
        val cond2 = if (lhs != lhs2) mhs2.map(_.toIf.cond) reduceLeft {
          (s1, s2) =>
            assert(s1 == s2, "conditions don't agree: " + s1 + "," + s2); s1
        }
        else t(c)
        val as2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (_.toIf.thenp)))
                   else (lhs zip as)) map { case (s, r) =>
          transformIfBody(s, r, t)
        }
        val bs2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (_.toIf.elsep)))
                   else (lhs zip bs)) map { case (s, r) =>
          transformIfBody(s, r, t)
        }

        printdbg(
          "came up with: " + lhs2 + ", if " + cond2 + " then " + as2 + " else " + bs2 + " with subst " + t.subst
            .mkString(",")
        )
      ...
      case TTP(lhs, mhs, SimpleFatLoop(s, x, rhs)) =>
        ...
        if (lhs != lhs2) {
          val missing =
            (lhs2.map(s => findDefinition(s).get) diff scope /*innerScope*/ )
          printdbg(
            "lhs changed! will add to innerScope: " + missing.mkString(",")
          )
          // innerScope = innerScope ::: missing
        ...
        val shape2 = if (lhs != lhs2) mhs2.map(_.toLoop.size) reduceLeft {
          (s1, s2) =>
            assert(s1 == s2, "shapes don't agree: " + s1 + "," + s2); s1
        }
        else t(s)
        val rhs2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (_.toLoop.body)))
                    else (lhs zip rhs)) map { case (s, r) =>
          transformLoopBody(s, r, t)
        }
        ...
        printdbg(
          "came up with: " + lhs2 + ", " + rhs2 + " with subst " + t.subst
            .mkString(",")
        )
        List(
          TTP(
            lhs2,
            mhs2,
            SimpleFatLoop(shape2, t(x).asInstanceOf[Sym[Int]], rhs2)
          )
        )
  ...
  def transformAllFully(
      currentScope0: List[Stm],
      result0: List[Exp[Any]],
      t: SubstTransformer
  ): (List[Stm], List[Exp[Any]]) = {
    ...
    def withEffectContext(body: => List[Stm]): List[Stm] = {
      ...
      if (leftovereffects.nonEmpty)
        printlog(
          "warning: transformation left effect context (will be discarded): " + leftovereffects
        )
      context = save
      scope
    }
  ...
  def simplify(
      scope: List[Stm]
  )(results: List[Exp[Any]]): (List[Stm], List[Exp[Any]]) = {

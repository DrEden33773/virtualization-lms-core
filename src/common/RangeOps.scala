...
trait RangeOps extends Base {
  ...
  class rangeOpsCls(r: Rep[Range]) {
    def foreach(f: Rep[Int] => Rep[Unit])(implicit pos: SourceContext) =
      range_foreach(r, f)
  }

  def infix_until(start: Rep[Int], end: Rep[Int])(implicit pos: SourceContext) =
  ...
  def range_until(start: Rep[Int], end: Rep[Int])(implicit
      pos: SourceContext
  ): Rep[Range]
  ...
  def range_foreach(r: Rep[Range], f: (Rep[Int]) => Rep[Unit])(implicit
      pos: SourceContext
  ): Rep[Unit]
}
...
trait RangeOpsExp extends RangeOps with PrimitiveOps with EffectExp {
  ...
  def range_until(start: Exp[Int], end: Exp[Int])(implicit
      pos: SourceContext
  ): Exp[Range] = Until(start, end)
  def range_start(r: Exp[Range])(implicit pos: SourceContext): Exp[Int] =
    r match {
      case Def(Until(start, end))                 => start
      case Def(Reflect(Until(start, end), u, es)) => start
      case _                                      => RangeStart(r)
    }
  def range_step(r: Exp[Range])(implicit pos: SourceContext): Exp[Int] =
    RangeStep(r)
  def range_end(r: Exp[Range])(implicit pos: SourceContext): Exp[Int] =
    r match {
      case Def(Until(start, end))                 => end
      case Def(Reflect(Until(start, end), u, es)) => end
      case _                                      => RangeEnd(r)
    }
  def range_foreach(r: Exp[Range], block: Exp[Int] => Exp[Unit])(implicit
      pos: SourceContext
  ): Exp[Unit] = {
    val i = fresh[Int]
  ...
  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
    ...
    case Reflect(RangeForeach(s, e, i, b), u, es) =>
      reflectMirrored(
        Reflect(
          RangeForeach(f(s), f(e), f(i).asInstanceOf[Sym[Int]], f(b)),
          mapOver(f, u),
          f(es)
        )
      )(mtyp1[A], pos)
    case Reflect(RangeStart(r), u, es) =>
      reflectMirrored(Reflect(RangeStart(f(r)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(RangeStep(r), u, es) =>
      reflectMirrored(Reflect(RangeStep(f(r)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(RangeEnd(r), u, es) =>
      reflectMirrored(Reflect(RangeEnd(f(r)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
    case Reflect(Until(s, e), u, es) =>
      reflectMirrored(Reflect(Until(f(s), f(e)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case Until(start, end) =>
      throw new GenerationFailedException(
        "OpenCLGenRangeOps: Range vector is not supported"
      )
    case RangeForeach(start, end, i, body) =>
...
trait CGenRangeOps extends CGenEffect with BaseGenRangeOps {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case Until(start, end) =>
      throw new GenerationFailedException(
        "CGenRangeOps: Range vector is not supported"
      )
    case RangeForeach(start, end, i, body) =>
      gen"""for(int $i=$start; $i < $end; $i++) {
           |${nestedBlock(body)}

...
trait ExceptionOpsExp extends ExceptionOps with EffectExp with StringOpsExp {
  ...
  def throw_exception(m: Exp[String]) =
    reflectEffect(ThrowException(m), Global())

  override def mirror[A: Typ](e: Def[A], f: Transformer)(implicit
      pos: SourceContext
  ): Exp[A] = (e match {
    case Reflect(ThrowException(s), u, es) =>
      reflectMirrored(Reflect(ThrowException(f(s)), mapOver(f, u), f(es)))(
        mtyp1[A],
        pos
      )
